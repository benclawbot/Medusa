...
The CI benchmark jobs compile the product acceptance runner and workspace test targets before starting the per-run timeout, then reuse that binary for the acceptance trials. The 600-second acceptance bound measures the authoritative test execution and recovery contract rather than a cold test-harness build; it provides evidence-based headroom over the 412,397-ms production-orchestration timing observed on the supported Linux runner. The pull-request coding-harness job is limited to non-draft PRs carrying the `final-issue-validation` label, so ordinary development iterations retain the fast scoring-contract check while final validation can request the full warm-up and two bounded trials. Its 45-minute outer limit covers that warm-up and evidence upload. The deterministic runtime job remains comprehensive with a 60-minute outer limit because it runs the warm-up, two bounded reliability trials, two orchestration trials, and two coding-harness trials before uploading evidence. These outer limits bound benchmark subprocesses while leaving time for setup and reporting.
