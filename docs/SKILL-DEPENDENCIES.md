...
Status: design and implementation contract for the next skill-lifecycle feature.
...
The first implementation must cover project-approved `.medusa/skills` only. Expanding dependency resolution to user-scoped or compatibility roots requires a separate lifecycle and precedence design so project skills cannot implicitly trust unmanaged instructions.
