...
| Refinement/learning state | `medusa-improvement` / authoritative | User/repository scoped | Exportable; secrets excluded | Tombstone then GC while preserving declared rollback/security evidence | Owning user/repository |
...
`.github/workflows/data-lifecycle-certification.yml` runs the lifecycle inventory plus the highest-value production storage/recovery suites on Linux, macOS, and Windows. Changes to durable-state owners, caches, frontends, evidence/artifacts, memory, checkpoints, time travel, refinement, configuration, or lifecycle policy trigger the gate.
