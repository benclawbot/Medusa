...
impl TimeTravelStore {
    ...
    pub fn restore(
        ...
        execution_id: &str,
        sequence: u64,
    ) -> Result<ExecutionState, TimeTravelError> {
        let entries = self
            .index
            .get(execution_id)
            .ok_or(TimeTravelError::MissingIndexEntry)?;
            ...
            .filter(|entry| entry.sequence < sequence)
        {
            ...
            for fingerprint in &entry.delta_fingerprints {
                let delta = self
                    .deltas
                    .get(fingerprint)
                    .ok_or(TimeTravelError::UnrestorableSequence)?;
                delta.verify()?;
    ...
    pub fn garbage_collect(
        ...
        self.deltas
            .retain(|fingerprint, _| retained_deltas.contains(fingerprint));
        Ok(removed)
    }
}
...
fn apply_delta(base: &FullSnapshot, delta: &StateDelta) -> Result<ExecutionState, TimeTravelError> {
    ...
    Ok(ExecutionState {
        execution_id: delta.execution_id.clone(),
...
mod tests {
    ...
    fn detects_delta_tampering() {
        ...
        assert_eq!(
            delta.verify(),
            Err(TimeTravelError::InvalidDeltaFingerprint)
        );
    }
    ...
    fn rejects_cross_execution_deltas() {
        ...
        assert_eq!(
            StateDelta::between(&base, &target),
            Err(TimeTravelError::ExecutionMismatch)
        );
    }
