...
impl CommitBarrier {
    ...
    pub fn record_vote(&mut self, vote: ParticipantVote) -> Result<(), &'static str> {
        ...
        if self
            .votes
            .iter()
            .any(|existing| existing.task_id == vote.task_id)
        {
        ...
        self.decision = if self
            .votes
            .iter()
            .any(|item| matches!(item.vote, Vote::Abort(_)))
        {
    ...
    pub fn validate_active_leases(
        ...
        let stale = self
            .votes
            .iter()
            .any(|vote| active_epochs.get(&vote.task_id).copied() != Some(vote.lease_epoch));
        if stale {
            self.decision = BarrierDecision::Abort;
            self.refresh_fingerprint();
        }
    ...
    pub fn prepared_transactions(&self) -> Result<Vec<String>, &'static str> {
        ...
        Ok(self
            .votes
            .iter()
            .map(|vote| vote.transaction_fingerprint.clone())
            .collect())
    }
...
mod tests {
    ...
    fn timeout_and_stale_lease_force_abort() {
        ...
        stale
            .validate_active_leases(&BTreeMap::from([("a".into(), 2)]))
            .unwrap();
