...
fn canonical_workers(workers: Vec<Worker>) -> Result<BTreeMap<String, Worker>, &'static str> {
    ...
    for mut worker in workers {
        ...
        if !unique(&worker.capabilities) {
            return Err("worker capabilities must be unique");
        }
...
fn validate_task(task: &Task) -> Result<(), &'static str> {
    ...
    if !unique(&task.dependencies) || !unique(&task.capabilities) || !unique(&task.write_paths) {
        return Err("task lists must contain unique values");
    }
