...
impl MemoryDocument {
    pub fn validate(&self) -> Result<(), &'static str> {
        if self.path.trim().is_empty() {
            return Err("memory path cannot be empty");
        }
        if !self.path.ends_with(".md") {
            return Err("memory document must be markdown");
        }
        if self.content.trim().is_empty() {
            return Err("memory content cannot be empty");
        }
        if self.revision == 0 {
            return Err("memory revision must start at one");
        }
        Ok(())
    }
}
...
impl RetrievalQuery {
    pub fn validate(&self) -> Result<(), &'static str> {
        if self.text.trim().is_empty() {
            return Err("retrieval query cannot be empty");
        }
        if self.maximum_results == 0 {
            return Err("maximum_results must be greater than zero");
        }
...
impl MemoryIndex {
    pub fn refresh(&mut self, document: MemoryDocument) -> Result<RefreshReport, &'static str> {
        ...
        let old_ids: Vec<String> = self
            .chunks
            .values()
            .filter(|chunk| chunk.path == document.path)
        ...
        self.documents
            .insert(document.path.clone(), document_fingerprint.clone());
        Ok(RefreshReport {
            path: document.path,
            changed: true,
            removed_chunks: old_ids.len(),
            added_chunks,
            document_fingerprint,
        })
    }
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        for chunk in self.chunks.values() {
            if !self.documents.contains_key(&chunk.path) {
                return Err("chunk references an unknown document");
            }
            if chunk.body.trim().is_empty() {
                return Err("chunk body cannot be empty");
            }
    ...
    pub fn retrieve(&self, query: &RetrievalQuery) -> Result<RetrievalResult, &'static str> {
        ...
        for required in &query.required_chunk_ids {
            if !self.chunks.contains_key(required) {
                return Err("required memory chunk does not exist");
            }
        }

        let query_terms = term_counts(&query.text);
        let mut ranked: Vec<MemoryHit> = self
            .chunks
            .values()
            .map(|chunk| {
                let mut score = 0u64;
                ...
                if score > 0 {
                    reasons.push("lexical_overlap".to_owned());
                }
                if query.preferred_paths.contains(&chunk.path) {
                    score += 2_000;
                    reasons.push("preferred_path".to_owned());
                }
                if query.required_chunk_ids.contains(&chunk.id) {
                    score += 1_000_000;
                    reasons.push("required".to_owned());
                }
                if chunk
                    .heading
                    .to_lowercase()
                ...
                {
                    score += 1_000;
                    reasons.push("heading_match".to_owned());
                }
                MemoryHit {
                    chunk: chunk.clone(),
                    score,
        ...
        for hit in ranked {
            ...
            if hits.len() >= query.maximum_results
                || selected_bytes.saturating_add(bytes) > query.maximum_bytes
            {
                if required {
                    return Err("required memory chunk cannot fit retrieval budget");
                }
        ...
        let query_fingerprint = fingerprint(
            format!(
                "{}\0{}\0{}\0{:?}\0{:?}",
                query.text,
                query.maximum_results,
                query.maximum_bytes,
                query.preferred_paths,
                query.required_chunk_ids
            )
            .as_bytes(),
        );
        let result_fingerprint = fingerprint(
            format!(
                "{}\0{}\0{:?}",
                index_fingerprint,
                query_fingerprint,
...
fn parse_markdown(document: &MemoryDocument) -> Result<Vec<MemoryChunk>, &'static str> {
    ...
    if sections.is_empty() {
        return Err("markdown document contains no retrievable content");
    }

    Ok(sections
        .into_iter()
        .enumerate()
        .map(|(ordinal, (heading, lines))| {
            let body = lines.join("\n").trim().to_owned();
            ...
            MemoryChunk {
                ...
                terms: term_counts(&format!("{heading} {body}")),
                id,
...
mod tests {
    ...
    fn doc(content: &str, revision: u64) -> MemoryDocument {
        MemoryDocument {
    ...
    fn required_chunk_fails_when_budget_is_too_small() {
        ...
        let error = index
            .retrieve(&RetrievalQuery {
                ...
                maximum_bytes: 1,
                preferred_paths: BTreeSet::new(),
                required_chunk_ids: BTreeSet::from([id]),
            })
            .unwrap_err();
