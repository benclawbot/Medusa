...
fn with_insecure_loopback_for_test(test_name: &str, test: impl FnOnce()) {
    ...
    // This fixture uses a local plaintext recorder. Opt in only in the child
    // process so the production HTTP policy remains enabled for the suite.
    let status = Command::new(std::env::current_exe().expect("tool sequence test executable"))
        .env("MEDUSA_ALLOW_INSECURE_PROVIDER_HTTP", "1")
        .args(["--exact", test_name, "--nocapture"])
        .status()
        .expect("spawn opted-in tool sequence test");
    assert!(
        status.success(),
        "opted-in tool sequence test failed: {status}"
    );
}
...
fn tool_results_do_not_emit_an_extra_empty_user_message() {
    with_insecure_loopback_for_test(
        "tool_results_do_not_emit_an_extra_empty_user_message",
        || {
            ...
            let server = thread::spawn(move || {
                ...
                let body: Value =
                    serde_json::from_slice(&bytes[header_end + 4..]).expect("request json");
        ...
        .expect("write response");
            ...
            let provider =
                OpenAiProvider::from_config_with_api_key(&config, Some("test-key".to_owned()))
