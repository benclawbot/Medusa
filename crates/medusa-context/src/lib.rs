...
impl ContextLedger {
    ...
    pub fn compact(&self, policy: CompactionPolicy) -> Result<CompactionResult, &'static str> {
        ...
        let protected: BTreeSet<&str> = self
            ...
            .iter()
            .filter(|item| {
                is_lossless_kind(item.kind)
                    || item.terminal
                    || recent_conversation.contains(item.id.as_str())
            })
            .map(|item| item.id.as_str())
            .collect();
...
mod tests {
    ...
    fn preserves_goals_blockers_and_evidence_losslessly() {
        ...
        ledger
            .append(item(1, ContextKind::Goal, "finish the repository"))
            .expect("append");
        ledger
            .append(item(2, ContextKind::Observation, "read file one"))
            .expect("append");
        ledger
            .append(item(3, ContextKind::Blocker, "permission required"))
            .expect("append");
        ledger
            .append(item(4, ContextKind::Evidence, "test output hash"))
            .expect("append");
        ledger
            .append(item(5, ContextKind::Observation, "read file two"))
            .expect("append");
        ...
        assert!(
            result
                .retained_items
                .iter()
                .any(|item| item.kind == ContextKind::Blocker)
        );
        assert!(
            result
                .retained_items
                .iter()
                .any(|item| item.kind == ContextKind::Evidence)
        );
    ...
    fn unknown_references_are_rejected() {
        ...
        assert_eq!(
            ledger.validate(),
            Err("context item references an unknown id")
        );
    }
    ...
    fn protected_context_cannot_be_silently_dropped() {
        let mut ledger = ContextLedger::default();
        ledger
            .append(item(1, ContextKind::Goal, &"x".repeat(1000)))
            .expect("append");
        assert_eq!(
            ledger.compact(CompactionPolicy {
                maximum_items: 1,
                maximum_bytes: 10,
                retain_recent_conversation: 0
            }),
