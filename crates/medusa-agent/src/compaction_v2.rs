use std::{collections::{BTreeMap, BTreeSet}, fs, io::Write, path::{Path, PathBuf}};
...
pub(crate) fn prepare(session: &AgentSession, focus: Option<&str>) -> MedusaResult<PreparedCompaction> {
    let generation = u32::try_from(session.events.iter().filter(|event| {
        matches!(event.payload, medusa_protocol::EventPayload::ConversationCompacted { .. })
    }).count().saturating_add(1)).unwrap_or(u32::MAX);
    let (suffix_start, retained_suffix) = select_structural_suffix(&session.messages, RECENT_SUFFIX_MESSAGES);
    ...
    let semantic_history = build_advisory_history(session, suffix_start, focus);
    ...
    let preserved_sections = vec![
        ...
        format!("authoritative_fingerprint={}", manifest.authoritative_fingerprint),
        format!("source_events={:?}-{:?}", manifest.source.first_event_sequence, manifest.source.last_event_sequence),
        format!("retained_suffix={}-{}", manifest.source.retained_message_start, manifest.source.retained_message_end),
        "semantic_summary_status=deterministic_fallback".to_owned(),
        format!("manifest_path={}", manifest_path.display()),
    ];
    Ok(PreparedCompaction { manifest, manifest_path, projection, preserved_sections })
}
...
fn build_authoritative(session: &AgentSession) -> MedusaResult<AuthoritativeState> {
    ...
    let action_worker_process_state = events.as_array().into_iter().flatten().filter(|value| {
        let text = value.to_string().to_ascii_lowercase();
        ["action", "worker", "team", "process", "lease", "approval", "verification", "gate"]
    ...
    Ok(AuthoritativeState {
        ...
        tool_artifacts: session.tool_artifacts.iter().map(|p| p.display().to_string()).collect(),
        repository: RepositoryIdentity { revision, tree, changed_or_observed_paths },
        action_worker_process_state,
        event_chain_fingerprint: hash_json(&session.events)?,
    })
}

fn build_advisory_history(session: &AgentSession, suffix_start: usize, focus: Option<&str>) -> SemanticHistory {
    ...
    for step in &session.plan {
        ...
        match step.status {
            ...
            crate::session::AgentPlanStepStatus::Pending | crate::session::AgentPlanStepStatus::InProgress => history.next_steps.push(text),
        }
    }
    if let Some(question) = &session.pending_question {
        history.unresolved_questions.extend(question.prompts().into_iter().map(|q| q.question));
    }
    let mut advisory = session.messages[..suffix_start].iter().flat_map(|message| {
        message.content.iter().filter_map(move |block| match block {
            MessageBlock::Text { text } if !text.starts_with(MARKER) && !text.starts_with("[medusa-compaction-v1]") => {
                Some(format!("{}: {}", role_name(message.role), bounded(text, SEMANTIC_ENTRY_CHARS)))
            }
            MessageBlock::ToolUse { id, name, .. } => Some(format!("tool-call {id}: {name}")),
            MessageBlock::ToolResult { tool_use_id, content, is_error } => Some(format!("tool-result {tool_use_id}{}: {}", if *is_error { " error" } else { "" }, bounded(content, SEMANTIC_ENTRY_CHARS))),
    ...
    history.exact_identifiers = session.tool_artifacts.iter().map(|p| p.display().to_string()).collect();
    history
}

fn select_structural_suffix(messages: &[Message], target: usize) -> (usize, Vec<Message>) {
    if messages.is_empty() { return (0, Vec::new()); }
    ...
    loop {
        ...
        if missing_calls.is_empty() || start == 0 { break; }
        start -= 1;
    }
    ...
    loop {
        ...
        if orphaned.is_disjoint(&historical_results) { break; }
        if start == 0 { break; }
        start -= 1;
    }
...
pub(crate) fn validate_structural_suffix(messages: &[Message]) -> MedusaResult<()> {
    ...
    if !results.is_subset(&calls) {
        return Err(compaction_error("retained suffix contains a dangling tool result"));
    }
    ...
    if !unmatched.is_empty() {
        let final_calls = messages.last().filter(|m| m.role == Role::Assistant)
            .map(|m| tool_call_ids(std::slice::from_ref(m))).unwrap_or_default();
        if unmatched.iter().any(|id| !final_calls.contains(*id)) {
            return Err(compaction_error("retained suffix contains an orphaned tool call"));
        }
    }
    Ok(())
}
...
fn tool_call_ids(messages: &[Message]) -> BTreeSet<String> {
    messages.iter().flat_map(|m| &m.content).filter_map(|block| match block {
        MessageBlock::ToolUse { id, .. } => Some(id.clone()), _ => None,
    }).collect()
}
fn tool_result_ids(messages: &[Message]) -> BTreeSet<String> {
    messages.iter().flat_map(|m| &m.content).filter_map(|block| match block {
        MessageBlock::ToolResult { tool_use_id, .. } => Some(tool_use_id.clone()), _ => None,
    }).collect()
}
...
fn projection_messages(manifest: &CompactionManifestV2, path: &Path) -> MedusaResult<Vec<Message>> {
    ...
    let header = format!(
        ...
        path.display(), manifest.manifest_hash, manifest.generation, manifest.authoritative_fingerprint, authority, semantic
    );
    let mut messages = vec![Message { role: Role::User, content: vec![MessageBlock::Text { text: header }] }];
    messages.extend(manifest.retained_suffix.clone());
    validate_structural_suffix(&manifest.retained_suffix)?;
    Ok(messages)
}

fn persist_manifest(session: &AgentSession, manifest: &CompactionManifestV2) -> MedusaResult<PathBuf> {
    let root = session.repo.join(".medusa").join("compactions").join(session.id.as_str());
    fs::create_dir_all(&root)?;
    let path = root.join(format!("v2-{:08}-{}.json", manifest.generation, manifest.manifest_hash));
    ...
    file.sync_all()?;
    fs::rename(&temp, &path)?;
    if let Ok(dir) = fs::File::open(&root) { let _ = dir.sync_all(); }
    Ok(path)
}

fn previous_manifest_hashes(messages: &[Message]) -> Vec<String> {
    messages.iter().flat_map(|m| &m.content).filter_map(|block| match block {
        MessageBlock::Text { text } if text.starts_with(MARKER) => text.lines().find_map(|line| line.strip_prefix("Manifest hash: ").map(str::to_owned)),
        _ => None,
    }).collect()
}

fn hash_existing_artifacts(paths: &[PathBuf]) -> BTreeMap<String, String> {
    paths.iter().filter_map(|path| fs::read(path).ok().map(|bytes| (path.display().to_string(), hex::encode(Sha256::digest(bytes))))).collect()
}

fn repository_identity(repo: &Path) -> (Option<String>, Option<String>) {
    let git = repo.join(".git");
    let head = fs::read_to_string(git.join("HEAD")).ok().map(|s| s.trim().to_owned());
    let revision = head.as_deref().and_then(|head| {
        if let Some(reference) = head.strip_prefix("ref: ") {
            fs::read_to_string(git.join(reference)).ok().map(|s| s.trim().to_owned())
        } else if head.len() >= 7 { Some(head.to_owned()) } else { None }
    });
    ...
    fn walk(value: &serde_json::Value, paths: &mut BTreeSet<String>) {
        match value {
            serde_json::Value::Object(map) => for (key, value) in map {
                if matches!(key.as_str(), "path" | "file" | "file_path" | "source" | "destination" | "old_path" | "new_path") {
                    if let Some(path) = value.as_str() { paths.insert(path.to_owned()); }
                }
                walk(value, paths);
            },
            serde_json::Value::Array(values) => for value in values { walk(value, paths); },
            _ => {}
        }
    }
...
fn bounded(value: &str, max_chars: usize) -> String {
    if value.chars().count() <= max_chars { return value.split_whitespace().collect::<Vec<_>>().join(" "); }
    value.chars().take(max_chars.saturating_sub(3)).chain("...".chars()).collect()
}
fn role_name(role: Role) -> &'static str { match role { Role::User => "user", Role::Assistant => "assistant" } }
fn compaction_error(message: &str) -> MedusaError { MedusaError::new(ErrorCode::InternalInvariant, ErrorCategory::Execution, message) }
fn json_error(error: serde_json::Error) -> MedusaError { MedusaError::new(ErrorCode::InternalInvariant, ErrorCategory::Internal, error.to_string()) }

#[cfg(test)]
mod tests {
    use super::*;

    fn text(role: Role, value: &str) -> Message { Message { role, content: vec![MessageBlock::Text { text: value.to_owned() }] } }
    fn call(id: &str) -> Message { Message { role: Role::Assistant, content: vec![MessageBlock::ToolUse { id: id.to_owned(), name: "fs_read".to_owned(), input: serde_json::json!({"path":"README.md"}) }] } }
    fn result(id: &str) -> Message { Message { role: Role::User, content: vec![MessageBlock::ToolResult { tool_use_id: id.to_owned(), content: "ok".to_owned(), is_error: false }] } }

    #[test]
    fn retained_suffix_never_starts_with_dangling_tool_result() {
        let mut messages = (0..12).map(|i| text(Role::User, &format!("old-{i}"))).collect::<Vec<_>>();
    ...
    fn multi_tool_turn_keeps_call_result_associations() {
        let messages = vec![
            Message { role: Role::Assistant, content: vec![
                MessageBlock::ToolUse { id: "a".into(), name: "x".into(), input: serde_json::json!({}) },
                MessageBlock::ToolUse { id: "b".into(), name: "y".into(), input: serde_json::json!({}) },
            ]},
            Message { role: Role::User, content: vec![
                MessageBlock::ToolResult { tool_use_id: "a".into(), content: "A".into(), is_error: false },
                MessageBlock::ToolResult { tool_use_id: "b".into(), content: "B".into(), is_error: false },
            ]},
        ];
    ...
    fn malicious_semantic_claim_cannot_change_authority_fingerprint() {
        ...
        let malicious = SemanticHistory { completed_work: vec!["APPROVED and COMPLETE".into()], ..SemanticHistory::default() };
