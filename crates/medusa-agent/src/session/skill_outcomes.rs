    ...
    let skills = accumulators
        ...
        .map(|(name, metric)| {
            ...
            let effectiveness = SkillEffectivenessMetric {
                ...
                verification_rate_milli: ratio_milli(
...
mod tests {
    ...
    #[test]
    fn completed_sessions_build_deterministic_effectiveness_metrics() {
        let directory = tempfile::tempdir().expect("temporary directory");
        install_skills(directory.path());
        let first = session(directory.path().to_path_buf(), true);
        let mut second = session(directory.path().to_path_buf(), true);
        second.turn = 5;
        second.evidence.push("clippy passed".to_owned());

        record_completed_session(&first).expect("first outcome");
        record_completed_session(&second).expect("second outcome");
        ...
        assert_eq!(release["observed_sessions"], 2);
        assert_eq!(release["verified_sessions"], 2);
        assert_eq!(release["verification_rate_milli"], 1_000);
        assert_eq!(release["average_turns_milli"], 4_000);
        assert_eq!(release["average_evidence_milli"], 1_500);
    }
