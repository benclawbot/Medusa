...
/// Runtime intensity for Medusa's minimal coding policy.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum CodingPolicyLevel {
    Off,
    Lite,
    Full,
    Ultra,
...
/// Resolves the active policy. Invalid or missing values fail safely to `full`.
pub(crate) fn active_level() -> CodingPolicyLevel {
    env::var("MEDUSA_CODING_POLICY")
        .ok()
        .and_then(|value| CodingPolicyLevel::from_str(&value))
        .unwrap_or(CodingPolicyLevel::Full)
}

/// Builds the system-prompt fragment applied to every implementation turn.
pub(crate) fn prompt_fragment() -> String {
    prompt_fragment_for(active_level())
}
...
fn prompt_fragment_for(level: CodingPolicyLevel) -> String {
    ...
    let intensity = match level {
        CodingPolicyLevel::Lite => {
            "Build what was requested, but mention a materially simpler alternative in one short line when one exists."
        }
        CodingPolicyLevel::Full => {
            "Enforce the ladder. Prefer the shortest correct diff and the fewest touched files."
        }
        CodingPolicyLevel::Ultra => {
            "Apply strict YAGNI. Prefer deletion over addition and challenge speculative requirements while still shipping the smallest useful result."
        }
...
mod tests {
    ...
    fn full_is_the_default_policy() {
        assert_eq!(CodingPolicyLevel::from_str("full"), Some(CodingPolicyLevel::Full));
