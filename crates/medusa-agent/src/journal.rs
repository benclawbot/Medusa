...
use crate::{
    ...
    session::{AgentSession, fallback_storage_root},
};
...
mod tests {
    ...
    #[test]
    fn legacy_snapshot_is_migrated_without_losing_events() {
        let directory = tempfile::tempdir().expect("repository");
        let mut legacy = session(directory.path());
        let created = next_event(
            &legacy,
            EventPayload::SessionCreated {
                objective: legacy.objective.clone(),
            },
        );
        legacy.events.push(created.clone());

        let outcome = load_or_migrate(directory.path(), &legacy.id, Some(legacy.clone()))
            .expect("migrate legacy snapshot");

        assert!(!outcome.repair_snapshot);
        assert_eq!(outcome.session.events, vec![created.clone()]);
        assert_eq!(
            replay_from_cursor(directory.path(), &legacy.id, 0).expect("replay migrated event"),
            vec![created]
        );
    }

    #[test]
    fn duplicate_event_id_is_idempotent_but_conflicting_reuse_fails_closed() {
        let directory = tempfile::tempdir().expect("repository");
        let mut current = session(directory.path());
        let created = next_event(
            &current,
        ...
        append_and_materialize(&mut current, created.clone());
        commit_snapshot(&current).expect("commit");

        assert_eq!(
            append_event(&current, &created).expect("idempotent replay"),
            AppendDisposition::Replayed
        );

        let mut conflicting = created;
        conflicting.payload = EventPayload::SessionResumed;
        conflicting.checksum = conflicting.compute_checksum().expect("recompute checksum");
        let error = append_event(&current, &conflicting).expect_err("reject conflicting reuse");
        assert!(error.message.contains("reused with conflicting content"));
    }

    #[test]
    fn state_only_snapshot_updates_are_committed() {
        let directory = tempfile::tempdir().expect("repository");
        let original = session(directory.path());
        commit_snapshot(&original).expect("initial commit");
        let mut updated = original.clone();
        updated.turn = 7;
        updated.pending_question = None;
        commit_snapshot(&updated).expect("state-only commit");

        let outcome = load_or_migrate(directory.path(), &updated.id, Some(original))
            .expect("load committed state");
        assert!(outcome.repair_snapshot);
        assert_eq!(outcome.session.turn, 7);
