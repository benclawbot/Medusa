...
pub enum ToolEffect { ReadOnly, Mutating, Destructive, ExternallyVisible }

#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
pub enum OutputMode { Compact, Normal, Verbatim }

#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub enum VerificationLevel { None, Syntax, Focused, Package, Workspace }
...
impl ToolBudgets {
    fn can_afford(&self, estimate: &ToolEstimate) -> bool {
        self.remaining_calls > self.reserved_verification_calls
            ...
            && self.remaining_output_tokens >= estimate.output_tokens.saturating_add(self.reserved_final_tokens)
...
pub struct ToolRegistry { tools: BTreeMap<String, ToolMetadata> }
...
impl ToolRegistry {
    ...
    pub fn recommend(&self, intent: &str, budgets: &ToolBudgets, allow_network: bool, allow_mutation: bool, mode: OutputMode, cache_hits: &BTreeSet<String>) -> Vec<ToolRecommendation> {
        let mut ranked = self.tools.values().filter(|tool| {
            ...
            let cache_bonus = if cache_hits.contains(&tool.name) { 10.0 } else { 0.0 };
            let risk = if tool.effect == ToolEffect::ReadOnly { 0.0 } else { 20.0 };
            ...
            ToolRecommendation {
                ...
                score,
                rationale: vec![format!("supports {intent}"), format!("estimated output {} tokens", tool.estimate.output_tokens)],
                output_mode: if tool.output_modes.contains(&mode) { mode } else { OutputMode::Normal },
                alternatives: Vec::new(),
            }
        }).collect::<Vec<_>>();
        ranked.sort_by(|left, right| right.score.total_cmp(&left.score).then_with(|| left.tool.cmp(&right.tool)));
        let names = ranked.iter().map(|item| item.tool.clone()).collect::<Vec<_>>();
        for item in &mut ranked {
            item.alternatives = names.iter().filter(|name| *name != &item.tool).take(3).cloned().collect();
        }
        ranked
    }
}
...
pub fn compact_failures(input: &str, limit: usize, expansion_handle: impl Into<String>) -> OutputEnvelope {
    let lines = input.lines().collect::<Vec<_>>();
    let failures = lines.iter().copied().filter(|line| {
        let value = line.to_ascii_lowercase();
        value.contains("error") || value.contains("failed") || value.contains("panic") || value.contains("assert")
    }).collect::<Vec<_>>();
    let retained = if failures.is_empty() { lines.iter().take(limit).copied().collect() } else { failures };
