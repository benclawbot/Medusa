...
impl Checkpoint {
    pub fn record(
        ...
        mut artifact_fingerprints: Vec<String>,
        attempt: u32,
    ) -> Result<Self, &'static str> {
        ...
        for value in &artifact_fingerprints {
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        let rebuilt = Self::record(
            ...
            self.artifact_fingerprints.clone(),
            self.attempt,
        )?;
        if rebuilt.fingerprint != self.fingerprint {
            return Err("checkpoint fingerprint does not match its contents");
        }
        Ok(())
    }
}
...
impl ExecutionState {
    pub fn start(
        ...
        let mut state = Self {
            execution_id,
            current_stage: ExecutionStage::Snapshot,
            snapshot_fingerprint,
            checkpoints: Vec::new(),
            failures: Vec::new(),
            attempt: 1,
    ...
    pub fn complete_stage(
        ...
        self.validate()?;
        if stage != self.current_stage || stage == ExecutionStage::Complete {
            return Err("stage completion is out of order");
        }
    ...
    pub fn record_failure(&mut self, failure: StageFailure) -> Result<(), &'static str> {
        self.validate()?;
        if failure.stage != self.current_stage {
            return Err("failure stage does not match current execution stage");
        }
        if failure.disposition == FailureDisposition::RetrySafe {
    ...
    pub fn resume(checkpoint: Checkpoint) -> Result<Self, &'static str> {
        ...
        let next = checkpoint
            .completed_stage
            .next()
            .ok_or("cannot resume after completion")?;
        let mut state = Self {
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        let expected = fingerprint(&(
            ...
            self.current_stage,
            self.snapshot_fingerprint.as_str(),
            &self.checkpoints,
            &self.failures,
            self.attempt,
        ))?;
...
pub fn finalize_manifest(
    ...
    transactions: &[String],
    result: &str,
) -> Result<ExecutionManifest, &'static str> {
    state.validate()?;
    if state.current_stage != ExecutionStage::Manifest
    ...
    ExecutionManifest::record(
        ...
        tool_outputs
            .iter()
            .map(|value| fingerprint_bytes(value.as_bytes()))
            .collect(),
        transactions.to_vec(),
...
mod tests {
    ...
    fn stages_advance_in_order_and_checkpoint() {
        ...
        state
            .complete_stage(ExecutionStage::Snapshot, vec![digest("snapshot-artifact")])
            .unwrap();
