...
use medusa_agent::{
    ...
    WorkerExecutionController, authoritative_verification_for_components_at,
    bind_session_to_delegation, prepare_components_for_verification,
};
            ...
            handles
                ...
                .map(|handle| {
                    handle.join().map_err(|_| {
                        "parallel implementer thread terminated unexpectedly".to_owned()
                    })?
                })
...
pub fn run_speculative_implementation(
    ...
    let mut ledger =
        SpeculationLedger::open_or_create(&ledger_path, &policy, repository_fingerprint.clone())?;
...
fn speculative_preflight(
    ...
    if contract.dependencies.len() < 2 {
        return Err(
            "speculative implementation requires at least two promotion dependencies".to_owned(),
        );
    }
...
fn preflight_promotion_conflict(preflight: &CoordinatorEvidence) -> Option<String> {
    ...
    preflight.workers.iter().find_map(|worker| {
        ...
        CONFLICT_MARKERS
            ...
            .map(|marker| {
                format!(
                    "{} reported promotion conflict marker `{marker}`",
                    worker.task_id
                )
            })
    })
...
fn promote_speculative_state(
    ...
    if current_branch(repo)? != state.speculation_branch {
        let detail =
            "primary repository branch changed while speculative work was running".to_owned();
        let _ = ledger.invalidate(InvalidationReason::RepositoryDrift, detail.clone());
        update_speculation_history(repo, ledger.record())?;
        return Err(detail);
    }
            ...
            ImplementationStatus::Running | ImplementationStatus::Retrying => {
                ...
                if controller.has_terminal_failure() {
                    ...
                    controller = create_implementation_controller(
                        &controller_path,
                        format!("{}-implementation-recovery-{}", plan.fingerprint, now_ms()?)
                            .as_str(),
                        &task,
                        &state.worker,
                    )?;
...
fn complete_prepared(
    ...
    let transaction = MutationTransaction::open_or_prepare(
        ...
        PreparedMutationInput {
            ...
            worktree_verification_receipt: state.verification_receipt.clone().ok_or_else(|| {
