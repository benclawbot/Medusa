    ...
    fn repository(scope: &str) -> (tempfile::TempDir, PathBuf, ProductionExecutionPlan, CoordinatorEvidence) {
    ...
    fn isolated_implementation_is_verified_integrated_and_cleaned() {
        ...
        assert_eq!(fs::read_to_string(repo.join("src/lib.rs")).unwrap(), "pub fn value() -> u32 { 2 }\n");
    ...
    fn cancellation_prevents_implementation_dispatch() {
        ...
        let error = coordinate_with_executor(
            ...
            &cancel,
            &events,
            |_| Err("executor must not run".to_owned()),
        )
