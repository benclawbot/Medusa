...
/// Display density for execution activity. `new` is the compact default:
/// keep the current action plus durable outcomes. `all` shows the meaningful
/// work log, while `verbose` also exposes diagnostic activity and expands
/// details. `off` hides transient tool/progress rows entirely.
