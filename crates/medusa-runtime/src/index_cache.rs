...
/// Runtime-owned repository index with deterministic invalidation semantics.
#[derive(Debug)]
...
impl RepositoryIndexCache {
    ...
    pub fn load(repo: PathBuf) -> Result<Self, RuntimeError> {
        let snapshot = IndexSnapshot::capture(&repo).map_err(RuntimeError::agent)?;
        let index = CodeIndex::build(&repo).map_err(RuntimeError::agent)?;
        Ok(Self {
            repo,
            snapshot,
            index,
    ...
    pub fn refresh(&mut self) -> Result<Option<IndexRefresh>, RuntimeError> {
        let newer = IndexSnapshot::capture(&self.repo).map_err(RuntimeError::agent)?;
    ...
    pub fn switch_repo(&mut self, repo: PathBuf) -> Result<(), RuntimeError> {
        if repo == self.repo {
            return Ok(());
        }
        *self = Self::load(repo)?;
