...
use crate::{
    ...
    mark_idle, prompt::PromptDraft, record_controller_event,
};
        ...
        match &event.payload {
            EventPayload::SessionActionAccepted { action } => {
                ...
                if action.kind == SessionActionKind::ReplaceFollowUp {
                    ...
                    let replaced = actions
                        ...
                        .ok_or_else(|| {
                            RuntimeError::agent("replacement action targets no admission")
                        })?;
            ...
            EventPayload::SessionActionLifecycleChanged {
                ...
                let action = actions
                    ...
                    .ok_or_else(|| {
                        RuntimeError::agent("session action transition has no admission")
                    })?;
            ...
            EventPayload::SessionActionTranscriptLinked {
                ...
                let action = actions
                    ...
                    .find(|candidate| candidate.action.action_id == *action_id)
                    .ok_or_else(|| {
                        RuntimeError::agent("session action transcript link has no admission")
                    })?;
                if action
                    .transcript_event_sequence
                    .replace(*transcript_event_sequence)
                    .is_some()
                {
                    return Err(RuntimeError::agent(
...
impl RuntimeController {
    ...
    fn ensure_safe_boundary_queue(&self, action: &SessionAction) -> Result<(), RuntimeError> {
        ...
        let session =
            load_session(&self.repo, &action.target_session_id).map_err(RuntimeError::agent)?;
    ...
    fn spawn_when_idle_action(&self, action: SessionAction) -> Result<(), RuntimeError> {
        ...
        thread::Builder::new()
            .name(format!(
                "medusa-session-action-{}",
                short_action_id(&action.action_id)
            ))
            .spawn(move || {
                loop {
            ...
            .map(|_| ())
            .map_err(|error| {
                RuntimeError::agent(format!("failed to spawn session action delivery: {error}"))
            })
    }
...
fn dispatch_when_idle(
    ...
    let result = (|| {
        ...
        if let Some(sequence) =
            find_committed_delivery(&repo, &action, expected_transcript_sequence)?
        {
        ...
        let sequence = find_committed_delivery(&repo, &action, expected_transcript_sequence)?
            .ok_or_else(|| {
                RuntimeError::agent(
                    "session action submission produced no provable durable user message",
                )
            })?;
        finish_message_delivery(&repo, &action, sequence)
    })();
...
fn action_text_payload(payload: &Value) -> Result<&str, RuntimeError> {
    payload
        ...
        .ok_or_else(|| {
            RuntimeError::InvalidCommand("session action requires non-empty text".to_owned())
        })
}
...
fn action_objective_payload(payload: &Value) -> Result<&str, RuntimeError> {
    payload
        ...
        .ok_or_else(|| {
            RuntimeError::InvalidCommand(
                "goal adjustment requires a non-empty objective".to_owned(),
            )
        })
}
...
fn transition_action(
    ...
    if !from.can_transition_to(to) {
        return Err(RuntimeError::agent(
            "invalid session action lifecycle transition",
        ));
    }
    record_controller_event(
        repo,
