...
pub(crate) struct ResolvedSkillGraph {
...
pub(crate) struct DependencyInspection {
    ...
    pub(crate) direct: Vec<String>,
    pub(crate) transitive_order: Vec<String>,
    pub(crate) reverse_dependents: Vec<String>,
}

pub(crate) fn resolve_project_skill(
    root: &Path,
    selected: &str,
    max_bytes: usize,
...
pub(crate) fn inspect_project_skill(
    root: &Path,
    selected: &str,
) -> Result<DependencyInspection, String> {
    validate_name(selected)?;
    let graph = load_graph(root)?;
    let direct = graph
        .get(selected)
    ...
    let reverse_dependents = reverse_dependents(&graph, selected)?;
...
pub(crate) fn validate_project_graph(root: &Path) -> Result<Vec<String>, String> {
    let graph = load_graph(root)?;
...
pub(crate) fn reverse_dependents(root: &Path, target: &str) -> Result<Vec<String>, String> {
    validate_name(target)?;
    let graph = load_graph(root)?;
    reverse_dependents(&graph, target)
}
...
fn load_graph(root: &Path) -> Result<BTreeMap<String, Vec<String>>, String> {
    ...
    let entries = fs::read_dir(&canonical_root)
        .map_err(|error| format!("read approved skill root {}: {error}", canonical_root.display()))?;
...
fn read_dependencies(root: &Path, name: &str) -> Result<Vec<String>, String> {
    ...
    if !canonical_manifest.starts_with(root) {
        return Err(format!("dependency manifest for `{name}` escapes approved skill root"));
    }
    ...
    let object = value
        .as_object()
        .ok_or_else(|| format!("{} must contain a JSON object", canonical_manifest.display()))?;
    ...
    for value in requires {
        let dependency = value.as_str().ok_or_else(|| {
            format!("{}.requires entries must be strings", canonical_manifest.display())
        })?;
