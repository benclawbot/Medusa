...
impl ConsensusRecord {
    ...
    pub fn cast_vote(&mut self, vote: Vote) -> Result<Option<String>, ConsensusError> {
        ...
        let voter = self
            .members
            .iter()
            .find(|m| m.id == vote.voter && m.voting)
            .ok_or_else(|| ConsensusError::UnknownVoter(vote.voter.clone()))?;
        let _ = voter;
        if !self
            .members
            .iter()
            .any(|m| m.id == vote.candidate && m.voting)
        {
...
mod tests {
    ...
    fn rejects_stale_terms_and_duplicate_votes() {
        ...
        assert_eq!(
            ...
            Err(ConsensusError::DuplicateVote("a".into()))
        );
    }
    ...
    fn only_elected_leader_advances_commit_monotonically() {
        ...
        assert_eq!(
            record.advance_commit("b", 6),
            Err(ConsensusError::CommitRegression)
        );
