...
//!
//! Audio capture/playback and provider transport remain outside the renderer.
//! This controller owns TUI interaction semantics and drives the shared
//! provider-neutral `RealtimeVoiceSession`.
...
pub enum PushToTalkMode {
    /// Full duplex is primary; holding Space temporarily focuses capture.
    HoldSpaceFocus,
    /// Used when the terminal cannot report key-release events reliably.
    TapSpaceFocus,
}
...
impl AudioEnvironment {
    ...
    pub fn availability(&self) -> VoiceAvailability {
        let reason = match self {
            Self::Local => return VoiceAvailability {
                available: true,
                reason: None,
            ...
            Self::Ssh => "voice is unavailable over SSH without explicitly forwarded local audio devices",
            Self::Container => "voice is unavailable in this container because no local audio device is exposed",
            Self::Wsl => "voice is unavailable in this WSL session until a working Windows audio bridge is detected",
            Self::Ci => "voice is disabled in CI; text mode remains fully available",
            Self::Headless => "voice is unavailable in this headless session because no local audio server is detected",
        };
...
impl TuiVoiceController {
    ...
    pub fn apply_control(&mut self, control: VoiceControl) -> Result<(), VoiceSessionError> {
        match control {
            ...
            VoiceControl::FocusCaptureStop => self.focus_capture = false,
            ...
            VoiceControl::CancelTask => {
                // The caller maps this to the same runtime cancellation used by text.
            }
        }
        Ok(())
    }

    /// Handles Space only when voice is active and the composer is empty.
    /// Normal text editing always wins.
    #[must_use]
    pub fn control_for_key(&self, key: KeyEvent, composer_empty: bool) -> VoiceControl {
        if !self.enabled || !composer_empty || key.code != KeyCode::Char(' ') {
...
mod tests {
    ...
    fn full_duplex_is_default_and_space_is_only_a_focus_override() {
        let mut voice = TuiVoiceController::new(available(), PushToTalkMode::HoldSpaceFocus)
            .expect("voice");
    ...
    fn tap_fallback_toggles_focus_for_release_less_terminals() {
        let mut voice = TuiVoiceController::new(available(), PushToTalkMode::TapSpaceFocus)
            .expect("voice");
        voice.enter().expect("enter");
        ...
        voice.apply_control(second).expect("stop");
        assert!(!voice.focus_capture);
    }
    ...
    fn unavailable_environment_never_activates_microphone() {
        ...
        assert!(voice.status_line().contains("unavailable"));
    }

    #[test]
    fn controls_keep_playback_response_and_task_cancellation_distinct() {
        assert_eq!(parse_voice_command("/stop-speech"), Some(VoiceControl::StopSpeech));
        assert_eq!(
            parse_voice_command("/cancel-response"),
            Some(VoiceControl::CancelResponse)
        );
        assert_eq!(parse_voice_command("/cancel-task"), Some(VoiceControl::CancelTask));
    }

    #[test]
    fn drop_and_leave_are_deterministic() {
        let mut voice = TuiVoiceController::new(available(), PushToTalkMode::HoldSpaceFocus)
            .expect("voice");
        voice.enter().expect("enter");
