...
impl ExecutionManifest {
    ...
    pub fn record(
        ...
        for digest in tool_output_fingerprints
            .iter()
            .chain(transaction_fingerprints.iter())
        {
            validate_digest(digest)?;
        }
...
pub fn compare_replay(
    ...
    compare_field(
        &mut divergences,
        ReplayDivergenceKind::FinalResult,
        &expected.final_result_fingerprint,
        &actual.final_result_fingerprint,
    );
    let reproducible = divergences.is_empty();
    let report_fingerprint = fingerprint(&(
...
fn canonical_files(
    files: impl IntoIterator<Item = SnapshotFile>,
) -> Result<Vec<SnapshotFile>, &'static str> {
...
mod tests {
    ...
    fn tampered_snapshot_is_rejected() {
        let mut snapshot =
            RepositorySnapshot::capture([SnapshotFile::from_bytes("a", b"a").unwrap()], None)
                .unwrap();
