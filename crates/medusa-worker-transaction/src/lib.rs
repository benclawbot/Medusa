...
use medusa_worker_read_set::{FileSnapshot, ReadSetValidation, WorkerReadSet, validate_read_set};
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        match self.operation {
            PatchOperation::Create
                if self.expected_before_fingerprint.is_some() || self.after_content.is_none() =>
            {
                Err("create patches require content and no before fingerprint")
            }
            PatchOperation::Replace
                if self.expected_before_fingerprint.is_none() || self.after_content.is_none() =>
            {
                Err("replace patches require content and a before fingerprint")
            }
            PatchOperation::Delete
                if self.expected_before_fingerprint.is_none() || self.after_content.is_some() =>
            {
...
pub fn validate_transaction(
    ...
    for patch in &transaction.patches {
        let actual = current
            .get(&patch.path)
            .map(|file| file.content_fingerprint.as_str());
...
mod tests {
    ...
    fn commits_when_read_set_and_patch_preconditions_match() {
        ...
        let transaction = PatchTransaction::plan(
            "tx-1",
            &read_set,
            [FilePatch {
    ...
    fn reports_overlapping_transaction_paths() {
        ...
        assert_eq!(
            conflicting_transactions([&first, &second])[0].2,
            vec!["src/lib.rs"]
        );
    }
