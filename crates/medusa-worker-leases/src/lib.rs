...
impl WorkerLease {
    pub fn acquire(
        ...
        if timeout_ms == 0 {
            return Err("lease timeout must be positive");
        }
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        let expected = hash(&(
            rebuilt.worker_id.as_str(),
            rebuilt.task_id.as_str(),
            rebuilt.lease_epoch,
        ...
        if self.heartbeat_at_ms < self.acquired_at_ms {
            return Err("heartbeat cannot predate acquisition");
        }
        Ok(())
    }
    ...
    pub fn heartbeat(&mut self, now_ms: u64) -> Result<(), &'static str> {
        self.validate()?;
        if now_ms < self.heartbeat_at_ms {
            return Err("heartbeat time cannot move backwards");
        }
...
impl LeaseRegistry {
    pub fn record(leases: impl IntoIterator<Item = WorkerLease>) -> Result<Self, &'static str> {
        ...
        for lease in leases {
            ...
            if by_task
                ...
                .is_some()
            {
                return Err("only one active lease is allowed per task");
            }
            if !worker_tasks.insert((lease.worker_id.clone(), lease.task_id.clone())) {
                return Err("duplicate worker lease");
            }
        }
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        if rebuilt.fingerprint != self.fingerprint {
            return Err("registry fingerprint does not match contents");
        }
        Ok(())
    }
...
pub fn deterministic_reassignment(
    task_id: &str,
    previous_worker: &str,
    ...
    let mut candidates = candidates
        .into_iter()
        .filter(|worker| worker != previous_worker)
        .collect::<BTreeSet<_>>();
    candidates
        .pop_first()
...
mod tests {
    ...
    fn reassignment_excludes_failed_worker_and_is_stable() {
        assert_eq!(
            deterministic_reassignment("task", "b", ["c".into(), "a".into(), "b".into()]).unwrap(),
            "a"
        );
    }
