...
impl TurnAssemblyInput {
    pub fn assemble(mut self) -> Result<TurnAssembly, &'static str> {
        ...
        let stable_tokens: usize = sections
            .iter()
...
fn validate_unique_names(
    ...
    for name in stable
        ...
        .chain(tools.iter().map(|item| item.name.as_str()))
    {
...
mod tests {
    ...
    fn context(id: &str, kind: ContextKind, sequence: u64, content: &str) -> ContextItem {
        ContextItem::new(id, kind, content, sequence, datetime!(2026-07-24 14:00 UTC))
            .expect("context")
    }
    ...
    fn optional_context_is_omitted_under_budget_pressure() {
        let mut value = input("task");
        value.retrieved_context.push(context(
            "observation",
    ...
    fn critical_context_never_silently_drops() {
        ...
        assert_eq!(
            value.assemble(),
            Err("execution-critical retrieved context does not fit the input budget")
        );
    }
