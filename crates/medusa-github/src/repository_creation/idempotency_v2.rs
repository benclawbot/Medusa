...
enum LedgerRecord {
    ...
    Receipt { receipt: Box<GitHubOperationReceiptV2> },
}
...
pub enum IdempotencyLookup {
    Miss,
    Replay(Box<GitHubOperationReceiptV2>),
    ResumeUncertain,
}
    ...
    pub fn lookup(
        ...
        for line in BufReader::new(file).lines() {
            ...
            match record {
                ...
                {
                    require_same_digest(&event.request_digest, request_digest)?;
                    matched_digest = true;
                    uncertain = matches!(
                        event.status,
                ...
                LedgerRecord::Receipt { receipt }
                    if receipt.idempotency_key_hash.as_deref() == Some(key_hash) =>
                {
                    require_same_digest(&receipt.request_digest, request_digest)?;
                    matched_digest = true;
                    uncertain = false;
                    if matches!(
        ...
        if let Some(receipt) = completed_receipt {
            Ok(IdempotencyLookup::Replay(Box::new(receipt.replayed())))
...
fn require_same_digest(recorded: &str, requested: &str) -> MedusaResult<()> {
...
mod tests {
    ...
    fn exact_key_and_digest_replays_prior_receipt() {
        ...
        assert!(matches!(
            ledger
                .lookup(
                    document.idempotency_key_hash().as_deref(),
                    &document.request_digest().expect("digest")
                )
                .expect("lookup"),
