...
impl MemoryObservation {
    pub fn validate(&self) -> Result<(), &'static str> {
        if self.id.trim().is_empty() || self.subject.trim().is_empty() || self.predicate.trim().is_empty() {
    ...
    pub fn key(&self) -> String {
        format!("{}\u{1f}{}\u{1f}{:?}", normalize(&self.subject), normalize(&self.predicate), self.kind)
    }
}
...
impl ConsolidationPolicy {
    pub fn validate(self) -> Result<Self, &'static str> {
        ...
        if self.minimum_confidence_basis_points > 10_000 || self.conflict_margin_basis_points > 10_000 {
...
pub fn consolidate(
    ...
    for observation in ordered {
        groups.entry(observation.key()).or_default().push(observation);
    }
    ...
    for (key, group) in groups {
        ...
        for observation in &group {
            candidates.entry(normalize(&observation.value)).or_default().push(observation);
        }

        let mut ranked: Vec<(String, Vec<&MemoryObservation>, u32)> = candidates
            .into_iter()
            .map(|(value, support)| {
                let score = support.iter().map(|item| u32::from(item.confidence_basis_points)).sum();
                (value, support, score)
            })
            .collect();
        ...
        if ranked.len() > 1 && margin < u32::from(policy.conflict_margin_basis_points) {
            let mut values = ranked.iter().map(|entry| entry.0.clone()).collect::<Vec<_>>();
            values.sort();
        ...
        if support_count < policy.minimum_support || mean_confidence < policy.minimum_confidence_basis_points {
            deferred.extend(group.iter().map(|item| item.id.clone()));
            continue;
        }

        let exemplar = winner.1[0];
        let mut support_ids = winner.1.iter().map(|item| item.id.clone()).collect::<Vec<_>>();
        support_ids.sort();
...
mod tests {
    ...
    fn repeated_evidence_consolidates_deterministically() {
        let input = vec![observation("b", "bash", 8_000), observation("a", "bash", 9_000)];
    ...
    fn close_competing_values_create_conflict() {
        ...
        let result = consolidate(
            &input,
            ConsolidationPolicy { minimum_support: 1, ..ConsolidationPolicy::default() },
        )
        .unwrap();
    ...
    fn weak_single_observation_is_deferred() {
        let result = consolidate(&[observation("a", "bash", 4_000)], ConsolidationPolicy::default()).unwrap();
        assert_eq!(result.deferred_observation_ids, vec!["a"]);
    }

    #[test]
    fn duplicate_ids_are_rejected() {
        let input = vec![observation("same", "bash", 8_000), observation("same", "bash", 8_000)];
