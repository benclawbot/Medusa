...
impl ExecutionTrace {
    pub fn new(
        ...
        leases: impl Into<String>,
        barrier: impl Into<String>,
        rollback_journal: Option<String>,
        task_outputs: BTreeMap<String, String>,
        final_snapshot: impl Into<String>,
        ...
        let mut trace = Self {
            ...
            rollback_journal,
            task_outputs,
            final_snapshot: final_snapshot.into(),
    ...
    fn validate_fields(&self) -> Result<(), &'static str> {
        ...
        if let Some(journal) = &self.rollback_journal {
            validate_digest(journal)?;
        }
        if self.task_outputs.keys().any(|id| id.trim().is_empty()) {
            return Err("task identifiers cannot be empty");
        }
        for digest in self.task_outputs.values() {
            validate_digest(digest)?;
        }
        Ok(())
    }
...
pub fn verify(
    ...
    if expected.execution_id != actual.execution_id {
        return Err("execution identifiers differ");
    }

    let mut divergences = Vec::new();
    compare_field(
        &mut divergences,
        DivergenceKind::InitialSnapshot,
        "repository",
        &expected.snapshot,
        &actual.snapshot,
    );
    ...
    compare_optional(
        &mut divergences,
        DivergenceKind::RollbackState,
        "rollback",
        expected.rollback_journal.as_ref(),
        actual.rollback_journal.as_ref(),
    );

    let ids = expected
        .task_outputs
        .keys()
        .chain(actual.task_outputs.keys())
        .cloned()
        .collect::<BTreeSet<_>>();
    for id in ids {
        match (expected.task_outputs.get(&id), actual.task_outputs.get(&id)) {
            (Some(left), Some(right)) if left != right => divergences.push(Divergence {
                kind: DivergenceKind::TaskOutput,
                subject: id,
            ...
            (None, Some(right)) => divergences.push(Divergence {
                kind: DivergenceKind::UnexpectedTask,
                subject: id,
                expected: None,
                actual: Some(right.clone()),
            }),
...
impl ReplayReport {
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        if self.equivalent != self.divergences.is_empty() {
            return Err("replay equivalence is inconsistent");
        }
        Ok(())
    }
}
...
fn validate_digest(value: &str) -> Result<(), &'static str> {
    ...
    {
        return Err("fingerprints must be lowercase SHA-256 hex");
    }
    Ok(())
}
fn hash<T: Serialize>(value: &T) -> String {
    hex::encode(Sha256::digest(
        serde_json::to_vec(value).expect("replay state serializes"),
    ))
}
...
mod tests {
    ...
    fn trace() -> ExecutionTrace {
        ExecutionTrace::new(
            ...
            digest('d'),
            None,
            BTreeMap::from([("task-a".into(), digest('e'))]),
            digest('f'),
        )
        .unwrap()
    }
    ...
    fn detects_tampered_trace_and_report() {
        ...
        assert_eq!(
            verify(&expected, &tampered),
            Err("execution trace fingerprint mismatch")
        );
