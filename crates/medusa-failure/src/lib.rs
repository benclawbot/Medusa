...
impl FailureHistory {
    ...
    pub fn classify(&self, signal: &FailureSignal, policy: RetryPolicy) -> FailureDecision {
        ...
        if matches!(signal.domain, FailureDomain::User | FailureDomain::Policy | FailureDomain::Permission) {
        ...
        if attempt >= policy.repeated_code_replan_threshold && !signal.transient {
            return FailureDecision {
                disposition: FailureDisposition::Replan,
                reason: "repeated non-transient failure indicates the strategy should change".to_owned(),
                attempt,
                remaining_attempts,
                backoff_ms: None,
            };
        ...
        if signal.transient || matches!(signal.domain, FailureDomain::Network | FailureDomain::Provider) {
            return FailureDecision {
        ...
        if matches!(signal.domain, FailureDomain::Tool | FailureDomain::Filesystem | FailureDomain::Internal) {
            return FailureDecision {
