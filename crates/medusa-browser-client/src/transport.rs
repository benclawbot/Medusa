...
/// A sidecar transport: write a serialized request, read one serialized response.
pub trait Transport: Write + Send {
    /// Read exactly one bounded newline-terminated frame.
    fn read_frame(&mut self, buf: &mut Vec<u8>, max_bytes: usize) -> io::Result<usize>;
}

/// Reads at most `max_bytes + 1` bytes before rejecting an oversized or unterminated frame.
