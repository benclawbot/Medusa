...
impl ProgressEvent {
    pub fn new(sequence: u64, kind: ProgressKind, message: impl Into<String>) -> MedusaResult<Self> {
        ...
        Ok(Self { sequence, kind, message, step_id: None, checkpoint_id: None })
    }
}
...
impl CheckpointStore {
    ...
    pub fn load_latest(&self, session_id: &SessionId) -> MedusaResult<Option<ExecutionCheckpoint>> {
        ...
        let mut paths = fs::read_dir(directory)?
            ...
            .collect::<Vec<_>>();
        paths.sort();
        let Some(path) = paths.pop() else { return Ok(None); };
...
fn validate_progress(sequence: u64, progress: &[ProgressEvent]) -> MedusaResult<()> {
    ...
    for event in progress {
        ...
        if previous.is_some_and(|value| event.sequence <= value) {
            return Err(validation("progress event sequence must be strictly increasing"));
        }
...
mod tests {
    ...
    fn latest_checkpoint_survives_restart() {
        ...
        for sequence in 1..=2 {
            let checkpoint = ExecutionCheckpoint::new(
                session.clone(), sequence, goal(), Vec::new(),
                vec![ProgressEvent::new(sequence, ProgressKind::CheckpointCreated, "saved").unwrap()],
                json!({"turn": sequence}),
            ).unwrap();
            store.save(&checkpoint).unwrap();
        }
        let restored = CheckpointStore::new(directory.path()).load_latest(&session).unwrap().unwrap();
    ...
    fn corrupted_checkpoint_is_rejected() {
        ...
        let store = CheckpointStore::new(directory.path());
        let session = SessionId::new();
        let checkpoint = ExecutionCheckpoint::new(session.clone(), 1, goal(), Vec::new(), Vec::new(), json!({})).unwrap();
    ...
    fn non_monotonic_progress_is_rejected() {
        ...
        assert!(ExecutionCheckpoint::new(SessionId::new(), 2, goal(), Vec::new(), events, json!({})).is_err());
    }
}
