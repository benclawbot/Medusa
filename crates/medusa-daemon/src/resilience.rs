...
use medusa_recovery_coordinator::{
    RecoveryAction, RecoveryCandidate, RecoveryCoordinator, RecoveryDecision,
};
...
impl ResilienceControlPlane {
    ...
    pub fn register(
        ...
        self.state
            ...
            .register(record)
            .map_err(|e| e.to_string())?;
        self.state.supervisors.insert(
            execution_id.clone(),
            SupervisorState::new(&execution_id).map_err(str::to_owned)?,
        );
        self.state
            .process_executions
        ...
        self.state
            ...
            .subscribe(WakeupSubscription {
                id: SubscriptionId::parse(format!("recover-{execution_id}"))
            ...
            .map_err(str::to_owned)?;
        self.state.events.push(SupervisionEvent::ProcessRegistered {
            process_id,
            execution_id,
        });
        self.persist()
    ...
    pub fn reconcile(
    ...
    ) -> Result<Vec<WakeupDelivery>, String> {
        ...
        for process_id in orphaned {
            ...
            let owned = self
                ...
                .registry
                .get(&process_id)
                .and_then(|record| record.owner_session.as_deref())
                ...
                if supervisor.resumable() {
                    supervisor
                        .apply(Signal::RecoveryRequired {
                            reason: "process missing or heartbeat stale".into(),
                        })
                        .map_err(str::to_owned)?;
                }
            }
    ...
    pub fn recover_once(
        ...
        if !self
            ...
            .insert(candidate.execution_id.clone())
        {
            return Ok(None);
        }
        let lock = self
            .recovery
            .acquire_lock(&candidate.transaction_id, &self.installation_id, 1)
            .map_err(|e| e.to_string())?;
        let decision = self
            .recovery
            .decide(candidate, &lock)
            .map_err(|e| e.to_string())?;
        ...
        if decision.action == RecoveryAction::Quarantine {
            self.state
                .events
                .push(SupervisionEvent::TerminalActionRequired {
                    execution_id: decision.execution_id.clone(),
                    reason: decision.reason.clone(),
                });
        }
    ...
    fn ensure_binding(&self, process_id: &ProcessId, execution_id: &str) -> Result<(), String> {
        ...
        let record = self
            ...
            .registry
            .get(process_id)
            .ok_or_else(|| "process is not registered".to_owned())?;
        if record.owner_session.as_deref() != Some(self.installation_id.as_str()) {
            return Err("process is not owned by this Medusa installation".into());
        }
        Ok(())
    ...
    fn persist(&self) -> Result<(), String> {
        let parent = self
            .path
            .parent()
            .ok_or_else(|| "control-plane path has no parent".to_owned())?;
...
mod tests {
    ...
    fn record(id: &str, owner: &str) -> ProcessRecord {
        let mut record = ProcessRecord::new(
            ProcessId::parse(id).unwrap(),
            ProcessSpec {
                program: "medusa-runtime".into(),
                args: vec![],
        ...
        .unwrap();
        record
            .mark_running(42, datetime!(2026-07-26 08:00 UTC))
            .unwrap();
        record
    }
    fn candidate() -> RecoveryCandidate {
        RecoveryCandidate {
            ...
            checkpoint_fingerprint: digest(1),
            snapshot_fingerprint: digest(2),
            replay_fingerprint: digest(3),
            rollback_fingerprint: Some(digest(4)),
        }
    }
    ...
    fn restart_reloads_registry_and_pending_wakeup_state() {
        ...
        plane
            .register(record("runtime-1", "install-a"), "exec-1")
            .unwrap();
        drop(plane);
        ...
        assert!(
            loaded
                .registry()
                .get(&ProcessId::parse("runtime-1").unwrap())
                .is_some()
        );
    }
    ...
    fn orphaning_wakes_once_and_recovery_is_deduplicated() {
        let dir = tempfile::tempdir().unwrap();
        let mut plane =
            ResilienceControlPlane::load_or_create(dir.path().join("state.json"), "install-a")
                .unwrap();
        plane
            .register(record("runtime-1", "install-a"), "exec-1")
            .unwrap();
    ...
    fn foreign_process_is_never_adopted_or_recovered() {
        ...
        assert!(
            plane
                .register(record("foreign", "install-b"), "exec-x")
                .is_err()
        );
    }
    ...
    fn terminal_decision_surfaces_user_action() {
        ...
        assert!(
            plane
                .events()
                .iter()
                .any(|event| matches!(event, SupervisionEvent::TerminalActionRequired { .. }))
        );
    }
}
