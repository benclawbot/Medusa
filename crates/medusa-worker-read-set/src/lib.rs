...
pub fn affected_workers<'a>(
    ...
    for read_set in read_sets {
        if read_set
            .files
            .iter()
            .any(|file| changed.contains(file.path.as_str()))
        {
...
mod tests {
    ...
    fn identifies_only_workers_affected_by_changed_paths() {
        let a = WorkerReadSet::record("a", "t1", [FileSnapshot::from_bytes("a.rs", b"a").unwrap()])
            .unwrap();
        let b = WorkerReadSet::record("b", "t2", [FileSnapshot::from_bytes("b.rs", b"b").unwrap()])
            .unwrap();
