...
impl RustSymbolTable {
    pub fn build(repo: &Path, ast: &RustAstIndex) -> MedusaResult<Self> {
        ...
        for (path, _) in ast.files() {
            table.index_file(path, &fs::read_to_string(repo.join(path))?)?;
        }
        table.normalize();
        Ok(table)
    }
    ...
    fn index_file(&mut self, path: &Path, source: &str) -> MedusaResult<()> {
        ...
        let root_id = scope_id(path, RustScopeKind::File, &qualified, None);
        self.scopes.insert(
            root_id.clone(),
            RustScope {
                id: root_id.clone(),
    ...
    fn insert_symbol(
        ...
        let scope_name = scope.qualified_name.clone();
        let ordinal = context.next_symbol(kind, &name);
...
impl<'a> Context<'a> {
    ...
    fn next_symbol(&mut self, kind: RustSymbolKind, name: &str) -> usize {
        let value = self
            .ordinals
            .entry((self.scope.clone(), kind, name.to_owned()))
            .or_default();
        let ordinal = *value;
        *value += 1;
        ordinal
