...
impl<'a> RustStructuredEditPlanner<'a> {
        ...
        match edit {
            ...
            RustAstEdit::InsertAfter { target, content } => {
                ...
                self.add_range_edit(
                    ...
                    node.range.end_byte,
                    content,
                    "insert_after_rust_node",
                );
            }
            RustAstEdit::ReplaceFunctionBody { function, body } => {
                let function_node =
                    self.resolve(&RustEditTarget::named("function_item", &function))?;
                let body_node = self.child_of_kind(&function_node, "block").ok_or_else(|| {
                    RustStructuredEditError::MissingTarget {
                        kind: "block".to_owned(),
                        name: Some(function),
                    }
                })?;
            ...
            RustAstEdit::ReplaceFunctionSignature {
                function,
                signature,
            } => {
                let function_node =
                ...
                self.add_range_edit(
                    &function_node,
                    function_node.range.start_byte,
                    body_node.range.start_byte,
        ...
        let matches = self
            ...
            .filter(|node| {
                target
                    .name
                    .as_ref()
                    .map_or(true, |name| self.node_name(node) == Some(name.as_str()))
            })
            .cloned()
            .collect::<Vec<_>>();
        match matches.len() {
            0 => Err(RustStructuredEditError::MissingTarget {
                kind: target.kind.clone(),
                name: target.name.clone(),
            }),
            1 => Ok(matches[0].clone()),
            count => Err(RustStructuredEditError::AmbiguousTarget {
                kind: target.kind.clone(),
                name: target.name.clone(),
                matches: count,
            }),
    ...
    fn remove_import(&mut self, path: &str) -> Result<(), RustStructuredEditError> {
        ...
        let matches = self
            ...
            .nodes_of_kind("use_declaration")
            .filter(|node| {
                self.node_text(node).map_or(false, |text| {
                    text.trim()
                        .trim_end_matches(';')
                        .trim_start_matches("use ")
                        == normalized
            ...
            .collect::<Vec<_>>();
        if matches.len() > 1 {
            return Err(RustStructuredEditError::AmbiguousTarget {
                kind: "use_declaration".to_owned(),
                name: Some(normalized.to_owned()),
                matches: matches.len(),
            });
        }
    ...
    fn add_module(&mut self, name: &str, public: bool) -> Result<(), RustStructuredEditError> {
        if !valid_identifier(name) {
            return Err(RustStructuredEditError::InvalidModuleName {
                name: name.to_owned(),
            });
        }
        if self
...
fn apply_text_edits(
    source: &str,
    edits: &[StructuredTextEdit],
) -> Result<String, RustStructuredEditError> {
    ...
    for edit in edits.into_iter().rev() {
        if output
            ...
            .is_none()
        {
            return Err(RustStructuredEditError::InvalidSourceRange {
                node_id: usize::MAX,
            });
        }
...
mod tests {
    ...
    fn edits_signature_and_body_with_ast_preconditions() {
        let mut planner =
            RustStructuredEditPlanner::new("rust-edit", "src/lib.rs", SOURCE).expect("planner");
        planner
    ...
    fn imports_and_modules_are_idempotent() {
        let mut planner =
            RustStructuredEditPlanner::new("imports", "src/lib.rs", SOURCE).expect("planner");
        planner
            .push(RustAstEdit::AddImport {
                path: "std::fmt::Debug".to_owned(),
            })
            .expect("import");
        planner
            .push(RustAstEdit::AddModule {
                name: "api".to_owned(),
                public: true,
            })
            .expect("module");
    ...
    fn missing_and_ambiguous_targets_fail_safely() {
        let mut planner =
            RustStructuredEditPlanner::new("missing", "src/lib.rs", SOURCE).expect("planner");
        assert!(matches!(
            planner.push(RustAstEdit::DeleteNode {
                target: RustEditTarget::named("function_item", "missing")
            }),
            Err(RustStructuredEditError::MissingTarget { .. })
        ));
        let mut planner = RustStructuredEditPlanner::new(
            "ambiguous",
            "src/lib.rs",
            "fn same() {}\nfn same() {}\n",
        )
        ...
        assert!(matches!(
            ...
            Err(RustStructuredEditError::AmbiguousTarget { .. })
        ));
    }
    ...
    fn malformed_staged_output_is_rejected() {
        let mut planner =
            RustStructuredEditPlanner::new("broken", "src/lib.rs", SOURCE).expect("planner");
        planner
            .push(RustAstEdit::ReplaceNode {
        ...
        assert!(matches!(
            planner.finish(),
            Err(RustStructuredEditError::MalformedOutput { .. })
        ));
    }
    ...
    fn representative_item_visibility_import_and_module_edits_reparse() {
        let mut planner =
            RustStructuredEditPlanner::new("representative", "src/lib.rs", SOURCE)
                .expect("planner");
        planner
        ...
        planner
            .push(RustAstEdit::AddImport {
                path: "std::collections::BTreeMap".to_owned(),
            })
            .expect("import");
        planner
            .push(RustAstEdit::AddModule {
    ...
    fn remove_import_reparses_cleanly() {
        let mut planner =
            RustStructuredEditPlanner::new("remove", "src/lib.rs", SOURCE).expect("planner");
        planner
            .push(RustAstEdit::RemoveImport {
                path: "std::fmt::Debug".to_owned(),
            })
            .expect("remove");
