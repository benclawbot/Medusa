...
pub enum RustScopeKind { File, Module, Type, Trait, Impl, Function, Block }

#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
pub enum RustSymbolKind {
    Function, Method, Struct, Enum, Variant, Trait, AssociatedType, Constant,
    Static, TypeAlias, Module, Field, Parameter, Local,
}
...
impl RustSymbolTable {
    ...
    #[must_use] pub fn symbol(&self, id: &RustSymbolId) -> Option<&RustSymbol> { self.symbols.get(id) }
    #[must_use] pub fn scope(&self, id: &RustScopeId) -> Option<&RustScope> { self.scopes.get(id) }
    #[must_use] pub fn qualified(&self, name: &str) -> Vec<&RustSymbol> { self.resolve(self.by_qualified_name.get(name)) }
    #[must_use] pub fn named(&self, name: &str) -> Vec<&RustSymbol> { self.resolve(self.by_simple_name.get(name)) }
    #[must_use] pub fn in_file(&self, path: &Path) -> Vec<&RustSymbol> { self.resolve(self.by_file.get(path)) }
    #[must_use] pub fn in_scope(&self, scope: &RustScopeId) -> Vec<&RustSymbol> { self.resolve(self.by_scope.get(scope)) }
    pub fn symbols(&self) -> impl Iterator<Item=&RustSymbol> { self.symbols.values() }
    pub fn scopes(&self) -> impl Iterator<Item=&RustScope> { self.scopes.values() }

    #[must_use]
    pub fn resolve_visible(&self, scope: &RustScopeId, name: &str) -> Option<&RustSymbol> {
        let mut current = Some(scope);
        while let Some(scope_id) = current {
            if let Some(found) = self.by_scope.get(scope_id).into_iter().flatten()
                .filter_map(|id| self.symbols.get(id)).find(|symbol| symbol.name == name) {
                return Some(found);
            }
            current = self.scopes.get(scope_id).and_then(|candidate| candidate.parent.as_ref());
        }
        None
    }

    fn resolve(&self, ids: Option<&Vec<RustSymbolId>>) -> Vec<&RustSymbol> {
        ids.into_iter().flatten().filter_map(|id| self.symbols.get(id)).collect()
    }
    ...
    fn index_file(&mut self, path: &Path, source: &str) -> MedusaResult<()> {
        ...
        let tree = parser.parse(source, None).ok_or_else(|| internal("Rust parser returned no tree"))?;
        let root = tree.root_node();
        let qualified = file_module_path(path);
        let root_id = make_scope_id(path, RustScopeKind::File, &qualified, None);
        self.scopes.insert(root_id.clone(), RustScope {
            id: root_id.clone(), parent: None, kind: RustScopeKind::File,
            name: qualified.rsplit("::").next().unwrap_or("crate").to_owned(),
            qualified_name: qualified, path: path.to_path_buf(), range: source_range(root), owner: None,
        });
    ...
    fn walk(&mut self, node: Node<'_>, context: &mut Context<'_>) -> MedusaResult<()> {
        ...
        if node.kind() == "impl_item" {
            ...
            let scope = self.insert_scope(node, RustScopeKind::Impl, name, context.owner.clone(), context)?;
            context.scope = scope;
            ...
            if let Some(name) = node_name(node, context.source) {
                ...
                if let Some(scope_kind) = owned_scope_kind(node.kind()) {
                    let scope = self.insert_scope(node, scope_kind, symbol.name.clone(), Some(symbol.id.clone()), context)?;
                    context.scope = scope;
            ...
            let scope = self.insert_scope(node, RustScopeKind::Block, name, context.owner.clone(), context)?;
            context.scope = scope;
            context.scope_kind = RustScopeKind::Block;
        }

        let mut cursor = node.walk();
        for child in node.named_children(&mut cursor) { self.walk(child, context)?; }
    ...
    fn insert_symbol(&mut self, node: Node<'_>, name: String, kind: RustSymbolKind, context: &mut Context<'_>) -> MedusaResult<RustSymbol> {
        let scope_id = context.scope.clone();
        let scope = self.scopes.get(&scope_id).ok_or_else(|| internal("symbol scope is missing"))?;
        ...
        let identity = format!("{}|{:?}|{}|{}|{}", context.path.display(), kind, scope_qualified, name, ordinal);
        let id = RustSymbolId(hash(identity.as_bytes()));
        let symbol = RustSymbol {
            id: id.clone(), scope: scope_id.clone(), owner: context.owner.clone(), kind,
            name: name.clone(), qualified_name: qualified_name.clone(), visibility: visibility(node, context.source),
            path: context.path.to_path_buf(), declaration: source_range(node), definition: source_range(node),
        };
        self.symbols.insert(id.clone(), symbol.clone());
        self.by_qualified_name.entry(qualified_name).or_default().push(id.clone());
        self.by_simple_name.entry(name).or_default().push(id.clone());
        self.by_file.entry(context.path.to_path_buf()).or_default().push(id.clone());
        self.by_scope.entry(scope_id).or_default().push(id);
        Ok(symbol)
    }

    fn insert_scope(&mut self, node: Node<'_>, kind: RustScopeKind, name: String, owner: Option<RustSymbolId>, context: &Context<'_>) -> MedusaResult<RustScopeId> {
        let parent = self.scopes.get(&context.scope).ok_or_else(|| internal("parent scope is missing"))?;
        let qualified_name = if kind == RustScopeKind::Impl {
            format!("{}::<{}>", parent.qualified_name, name)
        } else { format!("{}::{}", parent.qualified_name, name) };
        let id = make_scope_id(context.path, kind, &qualified_name, Some(&context.scope));
        self.scopes.insert(id.clone(), RustScope {
            id: id.clone(), parent: Some(context.scope.clone()), kind, name, qualified_name,
            path: context.path.to_path_buf(), range: source_range(node), owner,
        });
        Ok(id)
    }

    fn normalize(&mut self) {
        for ids in self.by_qualified_name.values_mut().chain(self.by_simple_name.values_mut())
            .chain(self.by_file.values_mut()).chain(self.by_scope.values_mut()) {
            ids.sort(); ids.dedup();
        }
    }
}
...
impl<'a> Context<'a> {
    fn new(path: &'a Path, source: &'a str, scope: RustScopeId, scope_kind: RustScopeKind) -> Self {
        Self { path, source, scope, scope_kind, owner: None, ordinals: BTreeMap::new(), block_ordinal: 0 }
    }
    fn next_symbol(&mut self, scope: RustScopeId, kind: RustSymbolKind, name: &str) -> usize {
        let value = self.ordinals.entry((scope, kind, name.to_owned())).or_default();
        let ordinal = *value; *value += 1; ordinal
    }
    fn next_block(&mut self) -> usize { let value = self.block_ordinal; self.block_ordinal += 1; value }
}
...
fn node_name(node: Node<'_>, source: &str) -> Option<String> {
    node.child_by_field_name("name").and_then(|name| source.get(name.byte_range())).map(str::to_owned)
}

fn impl_name(node: Node<'_>, source: &str) -> String {
    node.child_by_field_name("type").and_then(|value| source.get(value.byte_range()))
        .unwrap_or("anonymous").trim().to_owned()
}

fn binding_names(node: Node<'_>, source: &str) -> Vec<String> {
    let mut names = Vec::new(); collect_binding_names(node, source, &mut names); names.sort(); names.dedup(); names
}

fn collect_binding_names(node: Node<'_>, source: &str, names: &mut Vec<String>) {
    if node.kind() == "identifier" {
        if let Some(name) = source.get(node.byte_range()) {
            if !matches!(name, "self" | "Self") { names.push(name.to_owned()); }
        }
        return;
    }
    let mut cursor = node.walk();
    for child in node.named_children(&mut cursor) { collect_binding_names(child, source, names); }
}

fn visibility(node: Node<'_>, source: &str) -> Option<String> {
    node.child_by_field_name("visibility").and_then(|value| source.get(value.byte_range())).map(str::to_owned)
}

fn source_range(node: Node<'_>) -> SourceRange {
    let start = node.start_position(); let end = node.end_position();
    SourceRange { start_byte: node.start_byte(), end_byte: node.end_byte(),
        start: SourcePosition { row: start.row, column: start.column },
        end: SourcePosition { row: end.row, column: end.column } }
}

fn make_scope_id(path: &Path, kind: RustScopeKind, qualified_name: &str, parent: Option<&RustScopeId>) -> RustScopeId {
    let identity = format!("{}|{:?}|{}|{}", path.display(), kind, qualified_name, parent.map_or("root", |value| value.0.as_str()));
    RustScopeId(hash(identity.as_bytes()))
}

fn file_module_path(path: &Path) -> String {
    let mut parts = path.components().filter_map(|part| part.as_os_str().to_str()).map(str::to_owned).collect::<Vec<_>>();
    if parts.first().is_some_and(|value| value == "src") { parts.remove(0); }
    if let Some(last) = parts.last_mut() { *last = last.trim_end_matches(".rs").to_owned(); }
    if parts.last().is_some_and(|value| matches!(value.as_str(), "lib" | "main" | "mod")) { parts.pop(); }
    let mut qualified = vec!["crate".to_owned()]; qualified.extend(parts); qualified.join("::")
}
...
mod tests {
    ...
    fn indexes_modules_impls_trait_methods_locals_and_shadowing() {
        ...
        assert!(table.symbols().any(|symbol| symbol.kind == RustSymbolKind::Method));
        assert!(table.scopes().any(|scope| scope.kind == RustScopeKind::Impl));
    }
