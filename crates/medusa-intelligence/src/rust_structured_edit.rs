...
impl<'a> RustStructuredEditPlanner<'a> {
        ...
        match edit {
            ...
            RustAstEdit::ReplaceFunctionBody { function, body } => {
                ...
                let body_node = self.child_of_kind(function_node, "block")
                    .ok_or_else(|| RustStructuredEditError::MissingTarget {
                        kind: "block".to_owned(),
            ...
            RustAstEdit::ReplaceFunctionSignature { function, signature } => {
                ...
                let end = body_node.range.start_byte;
                self.add_range_edit(function_node, function_node.range.start_byte, end, format!("{} ", signature.trim()), "replace_function_signature");
            }
            RustAstEdit::SetVisibility { target, visibility } => {
                ...
                let replacement = replace_visibility(text, &visibility);
                self.add_node_edit(node, replacement, "set_rust_visibility");
            }
    ...
    pub fn finish(mut self) -> Result<StructuredEditPlan, RustStructuredEditError> {
        ...
        if reparsed.has_errors() {
            return Err(RustStructuredEditError::MalformedOutput {
                diagnostics: reparsed.diagnostics.len(),
            });
        }
        Ok(self.plan)
    }
    ...
    fn resolve(&self, target: &RustEditTarget) -> Result<&RustAstNode, RustStructuredEditError> {
        if let Some(node_id) = target.node_id {
            return self.document.node(node_id)
                .ok_or(RustStructuredEditError::InvalidNodeId { node_id });
        }
        let mut matches = self.document.nodes.iter()
            .filter(|node| target.kind.is_empty() || node.kind == target.kind)
            .filter(|node| target.name.as_ref().is_none_or(|name| self.node_name(node) == Some(name.as_str())))
            .collect::<Vec<_>>();
        matches.sort_by_key(|node| node.range.start_byte);
        match matches.len() {
    ...
    fn node_name<'b>(&'b self, node: &'b RustAstNode) -> Option<&'b str> {
        node.children.iter().filter_map(|id| self.document.node(*id)).find_map(|child| {
            matches!(child.kind.as_str(), "identifier" | "type_identifier" | "field_identifier")
                .then(|| self.source.get(child.range.start_byte..child.range.end_byte))
                .flatten()
        })
    }

    fn child_of_kind<'b>(&'b self, node: &'b RustAstNode, kind: &str) -> Option<&'b RustAstNode> {
        node.children.iter().filter_map(|id| self.document.node(*id)).find(|child| child.kind == kind)
    }
    ...
    fn add_range_edit(&mut self, node: &RustAstNode, start: usize, end: usize, replacement: String, intent: &str) {
        let expected = self.source.get(start..end).unwrap_or_default().to_owned();
    ...
    fn add_import(&mut self, path: &str) -> Result<(), RustStructuredEditError> {
        ...
        let existing = self.document.nodes_of_kind("use_declaration")
            .filter_map(|node| self.node_text(node).ok())
            .any(|text| text.trim().trim_end_matches(';').trim_start_matches("use ") == normalized);
        if existing { return Ok(()); }
        let mut imports = self.document.nodes_of_kind("use_declaration").collect::<Vec<_>>();
        ...
        let synthetic = self.document.node(self.document.root).expect("root node");
        self.add_range_edit(synthetic, insertion, insertion, format!("{prefix}use {normalized};\n"), "add_rust_import");
        Ok(())
    }
    ...
    fn remove_import(&mut self, path: &str) -> Result<(), RustStructuredEditError> {
        ...
        let matches = self.document.nodes_of_kind("use_declaration")
            .filter(|node| self.node_text(node).is_ok_and(|text| text.trim().trim_end_matches(';').trim_start_matches("use ") == normalized))
            .collect::<Vec<_>>();
        if matches.len() > 1 {
    ...
    fn add_module(&mut self, name: &str, public: bool) -> Result<(), RustStructuredEditError> {
        ...
        let exists = self.document.nodes_of_kind("mod_item")
            .any(|node| self.node_name(node) == Some(name));
        if exists { return Ok(()); }
        let root = self.document.node(self.document.root).expect("root node");
...
mod tests {
    ...
    fn imports_and_modules_are_idempotent() {
        ...
        planner.push(RustAstEdit::AddImport { path: "std::fmt::Debug".to_owned() }).expect("existing import");
        planner.push(RustAstEdit::AddModule { name: "api".to_owned(), public: true }).expect("existing module");
        let plan = planner.finish().expect("plan");
    ...
    fn missing_and_ambiguous_targets_fail_safely() {
        ...
        let error = planner.push(RustAstEdit::DeleteNode {
            target: RustEditTarget::named("function_item", "missing"),
        }).expect_err("missing");
        assert!(matches!(error, RustStructuredEditError::MissingTarget { .. }));

        let duplicate = "fn same() {}\nfn same() {}\n";
        let mut planner = RustStructuredEditPlanner::new("ambiguous", "src/lib.rs", duplicate).expect("planner");
        let error = planner.push(RustAstEdit::DeleteNode {
            target: RustEditTarget::named("function_item", "same"),
        }).expect_err("ambiguous");
        assert!(matches!(error, RustStructuredEditError::AmbiguousTarget { .. }));
    }
    ...
    fn malformed_staged_output_is_rejected() {
        ...
        planner.push(RustAstEdit::ReplaceNode {
            target: RustEditTarget::named("function_item", "answer"),
            replacement: "fn answer( {".to_owned(),
        }).expect("edit");
        assert!(matches!(planner.finish(), Err(RustStructuredEditError::MalformedOutput { .. })));
    }

    #[test]
    fn representative_visibility_import_and_module_edits_reparse() {
        let mut planner = RustStructuredEditPlanner::new("representative", "src/lib.rs", SOURCE).expect("planner");
        planner.push(RustAstEdit::SetVisibility {
            target: RustEditTarget::named("function_item", "answer"),
            visibility: "pub(crate)".to_owned(),
        }).expect("visibility");
        planner.push(RustAstEdit::RemoveImport { path: "std::fmt::Debug".to_owned() }).expect("remove import");
        planner.push(RustAstEdit::AddImport { path: "std::collections::BTreeMap".to_owned() }).expect("add import");
        planner.push(RustAstEdit::AddModule { name: "domain".to_owned(), public: false }).expect("module");
        let plan = planner.finish().expect("plan");
        assert_eq!(plan.text_edits.len(), 4);
    }
}
