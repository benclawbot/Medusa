...
pub enum ExperienceScope { Global, Language, Repository }

#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum LearningAction { Ignore, Remember, CreateSkill, PatchSkill }
...
pub trait LearningPolicy: Send + Sync {
    fn decide(&self, experience: &ExperienceRecord, matched_skill: Option<&str>) -> MedusaResult<LearningAction>;
}
...
impl Default for RuleBasedLearningPolicy {
    fn default() -> Self { Self { minimum_complexity: 5, minimum_confidence_milli: 700 } }
}
...
impl LearningPolicy for RuleBasedLearningPolicy {
    fn decide(&self, experience: &ExperienceRecord, matched_skill: Option<&str>) -> MedusaResult<LearningAction> {
        ...
        Ok(if matched_skill.is_some() { LearningAction::PatchSkill } else { LearningAction::CreateSkill })
    }
}

pub struct ExperienceCompiler<P> { policy: P }

impl<P: LearningPolicy> ExperienceCompiler<P> {
    #[must_use]
    pub fn new(policy: P) -> Self { Self { policy } }

    pub fn propose(&self, experience: &ExperienceRecord, candidate: &SkillCandidate) -> MedusaResult<LearningProposal> {
        let action = self.policy.decide(experience, candidate.matched_existing_skill.as_deref())?;
        let mut evidence = experience.verification.iter().filter(|item| item.passed)
            .map(|item| format!("{}: {}", item.command, item.summary)).collect::<Vec<_>>();
        evidence.extend(experience.user_corrections.iter().cloned());
        Ok(LearningProposal {
            id: Ulid::new().to_string(),
            action,
            experience_id: experience.id.clone(),
            why_learn: format!("session {} produced a reusable procedure with complexity {}", experience.session_id, experience.complexity_score()),
            ...
            requires_approval: matches!(action, LearningAction::CreateSkill | LearningAction::PatchSkill),
        })
    }
}
...
pub struct InMemorySessionRecall { documents: Vec<SessionSearchDocument> }

impl InMemorySessionRecall {
    pub fn insert(&mut self, document: SessionSearchDocument) { self.documents.push(document); }

    #[must_use]
    pub fn search(&self, query: &SessionSearchQuery) -> Vec<SessionSearchHit> {
        let terms = query.query.split_whitespace().map(str::to_ascii_lowercase).collect::<Vec<_>>();
        let mut hits = self.documents.iter().filter(|document| {
            query.repository_fingerprint.as_ref().is_none_or(|value| &document.repository_fingerprint == value)
                && query.tool.as_ref().is_none_or(|value| document.tools.contains(value))
                && query.outcome.as_ref().is_none_or(|value| &document.outcome == value)
        }).filter_map(|document| {
...
impl DelegationProfile {
    pub fn validate(&self) -> MedusaResult<()> {
        if self.name.trim().is_empty() || self.tools.is_empty() { return Err(invalid("delegation profile requires a name and tools")); }
        Ok(())
    }
}

pub trait MemoryProvider: Send + Sync { fn remember(&self, record: &ExperienceRecord) -> MedusaResult<()>; }
pub trait ContextEngine: Send + Sync { fn compress(&self, summary: &CodingContextSummary) -> MedusaResult<String>; }
pub trait RetrievalProvider: Send + Sync { fn search(&self, query: &SessionSearchQuery) -> MedusaResult<Vec<SessionSearchHit>>; }
pub trait ExecutionBackend: Send + Sync { fn execute(&self, profile: &DelegationProfile, task: &str) -> MedusaResult<String>; }
...
impl PromptFingerprint {
    ...
    pub fn new(base_prompt: &str, skill_catalogue: &str, project_context: &str) -> Self {
        Self { base_prompt_sha256: digest_text(base_prompt), skill_catalogue_sha256: digest_text(skill_catalogue), project_context_sha256: digest_text(project_context) }
    }
}

fn digest_text(value: &str) -> String { hex_digest(Sha256::digest(value.as_bytes())) }
fn digest_json<T: Serialize>(value: &T) -> MedusaResult<String> { Ok(hex_digest(Sha256::digest(serde_json::to_vec(value)?))) }
fn hex_digest(bytes: impl AsRef<[u8]>) -> String { bytes.as_ref().iter().map(|byte| format!("{byte:02x}")).collect() }
...
mod tests {
    ...
    fn verified_experience() -> ExperienceRecord {
        ...
        record.successful_procedure = vec!["stop running process".to_owned(), "replace executable from helper".to_owned()];
        ...
        record.verification = vec![VerificationEvidence { command: "cargo test -p medusa-update".to_owned(), passed: true, summary: "all update tests passed".to_owned() }];
    ...
    fn compiler_patches_matching_skill_and_requires_approval() {
        let proposal = ExperienceCompiler::new(RuleBasedLearningPolicy::default()).propose(&verified_experience(), &SkillCandidate {
            name: "windows-update".to_owned(), summary: "Safely update Windows binaries".to_owned(),
            instructions: "Stop processes, then replace from a helper.".to_owned(), supporting_files: Vec::new(),
            scope: ExperienceScope::Global, matched_existing_skill: Some("windows-update@3".to_owned()),
        }).expect("proposal");
        assert_eq!(proposal.action, LearningAction::PatchSkill);
        assert!(proposal.requires_approval);
        assert_eq!(proposal.rollback_revision.as_deref(), Some("windows-update@3"));
    }

    #[test]
    fn recall_filters_and_ranks_exact_prior_solutions() {
        let mut recall = InMemorySessionRecall::default();
        recall.insert(SessionSearchDocument { session_id: "a".to_owned(), parent_session_id: None,
            repository_fingerprint: "repo".to_owned(), text: "fixed Windows Cargo executable replacement with detached helper".to_owned(),
            tools: BTreeSet::from(["shell".to_owned()]), outcome: "success".to_owned() });
        let hits = recall.search(&SessionSearchQuery { query: "Windows Cargo replacement".to_owned(),
            repository_fingerprint: Some("repo".to_owned()), tool: Some("shell".to_owned()), outcome: Some("success".to_owned()) });
    ...
    fn prompt_fingerprint_changes_only_for_changed_layer() {
        ...
        assert_ne!(baseline.skill_catalogue_sha256, changed.skill_catalogue_sha256);
        assert_eq!(baseline.project_context_sha256, changed.project_context_sha256);
    }
}
