...
impl MemoryDocument {
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        if self
            ...
            .as_ref()
            .is_some_and(|expected| expected != &fingerprint_bytes(self.content.as_bytes()))
        {
...
pub fn plan_writeback(
    ...
    canonical_memories.sort_by(|a, b| {
        a.kind
            .cmp(&b.kind)
    ...
    canonical_conflicts.sort_by(|a, b| {
        a.key
            .cmp(&b.key)
    ...
    for conflict in &canonical_conflicts {
        if conflict.key.trim().is_empty() || conflict.fingerprint.trim().is_empty() {
            return Err("memory conflict fields cannot be empty");
        }
        if !conflict_fingerprints.insert(conflict.fingerprint.as_str()) {
            return Err("memory conflict fingerprints must be unique");
        }
    }
    ...
    if policy.include_conflicts && !canonical_conflicts.is_empty() {
        grouped.entry(policy.default_path.clone()).or_default();
    }

    let mut patches = Vec::new();
    for (path, path_memories) in grouped {
        validate_path(&path)?;
        let path_conflicts: &[MemoryConflict] =
            if policy.include_conflicts && path == policy.default_path {
                &canonical_conflicts
            } else {
                &[]
            };
        ...
        let before_fingerprint =
            existing.map(|document| fingerprint_bytes(document.content.as_bytes()));
    ...
    let unresolved_conflict_fingerprints = canonical_conflicts
        .iter()
...
fn render_generated(
    ...
    for memory in memories {
        ...
        output.push_str(&format!(
            "- **{} — {}:** {}",
            memory.subject.trim(),
            memory.predicate.trim(),
            memory.value.trim()
        ));
        if include_provenance {
            output.push_str(&format!(
                " <!-- support:{} confidence:{} fingerprint:{} -->",
                memory.support_ids.join(","),
                memory.confidence_basis_points,
                memory.fingerprint
            ));
        }
        output.push('\n');
    ...
    if !conflicts.is_empty() {
        ...
        for conflict in conflicts {
            output.push_str(&format!(
                "- **{}:** {} <!-- conflict:{} -->\n",
                conflict.key,
                conflict.candidate_values.join(" | "),
