//! Durable goal contracts and evidence-based completion assessment.
...
impl GoalContract {
    ...
    pub fn evaluate(&self, evidence: &[CompletionEvidence]) -> CompletionAssessment {
        ...
        let criteria = self
            ...
            .iter()
            .map(|criterion| {
                let observed_evidence = observed.get(&criterion.id).cloned().unwrap_or_default();
                let missing_evidence = criterion
                    .required_evidence
                    .difference(&observed_evidence)
                    .copied()
                    .collect::<BTreeSet<_>>();
                CriterionAssessment {
...
mod tests {
    ...
    #[test]
    fn completion_requires_all_declared_evidence() {
        ...
        let partial = vec![
            CompletionEvidence::new(id("behavior-fixed"), EvidenceKind::Test, "test passed")
                .expect("evidence"),
            CompletionEvidence::new(id("build-clean"), EvidenceKind::Build, "build passed")
                .expect("evidence"),
        ];
        assert!(!contract.evaluate(&partial).complete);
    ...
    #[test]
    fn empty_criteria_never_complete() {
    ...
    fn duplicate_ids_are_rejected() {
        let criterion = AcceptanceCriterion::new(id("same"), "condition", [EvidenceKind::Test])
            .expect("criterion");
        assert_eq!(
