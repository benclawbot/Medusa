//! Evidence-backed learning primitives for turning completed work into reviewable knowledge.
...
impl InMemorySessionRecall {
    ...
    pub fn search(&self, query: &SessionSearchQuery) -> Vec<SessionSearchHit> {
        ...
        let mut hits = self
            ...
            .iter()
            .filter(|document| {
                query.repository_fingerprint.as_ref().is_none_or(|fingerprint| {
                    &document.repository_fingerprint == fingerprint
                }) && query
                    .tool
                    .as_ref()
                    .is_none_or(|tool| document.tools.contains(tool))
                    && query
                        .outcome
                        .as_ref()
                        .is_none_or(|outcome| &document.outcome == outcome)
            })
            .filter_map(|document| {
...
mod tests {
    ...
    fn compiler_patches_matching_skill_and_requires_approval() {
        let compiler = ExperienceCompiler::new(RuleBasedLearningPolicy::default());
