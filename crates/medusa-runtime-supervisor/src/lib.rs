...
impl SupervisorState {
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        for digest in [
            self.schedule_fingerprint.as_ref(),
            self.lease_fingerprint.as_ref(),
            self.barrier_fingerprint.as_ref(),
            self.journal_fingerprint.as_ref(),
            self.final_snapshot.as_ref(),
            self.replay_report_fingerprint.as_ref(),
        ]
        .into_iter()
        .flatten()
        {
...
mod tests {
    ...
    fn completes_the_full_execution_lifecycle() {
        ...
        state
            ...
            .unwrap();
        state
            .apply(Signal::LeasesReady {
                fingerprint: digest('b'),
            })
            .unwrap();
        state.apply(Signal::WorkersStarted).unwrap();
        state
            .apply(Signal::BarrierPrepared {
                fingerprint: digest('c'),
            })
            .unwrap();
        state
            .apply(Signal::CommitStarted {
                journal_fingerprint: digest('d'),
            })
            .unwrap();
        state
            .apply(Signal::CommitApplied {
                final_snapshot: digest('e'),
            })
            .unwrap();
        state
            .apply(Signal::ReplayVerified {
                report_fingerprint: digest('f'),
            })
            .unwrap();
