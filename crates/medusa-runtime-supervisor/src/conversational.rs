...
pub enum SupervisorEvent {
    ...
    ConflictDetected { first_task_id: String, second_task_id: String, files: Vec<String> },
    ApprovalRequested { binding: ApprovalBinding },
    ApprovalResolved { task_id: String, approval_id: String, approved: bool },
    CancelAllRequested { task_ids: Vec<String> },
}
...
impl ConversationalSupervisor {
    pub fn new(conversation_id: impl Into<String>) -> Result<Self, &'static str> {
        ...
        Ok(Self { conversation_id, tasks: BTreeMap::new(), recent_task_ids: Vec::new(), sequence: 0 })
    }

    pub fn restore(snapshot: RegistrySnapshot) -> Result<Self, &'static str> {
        require_non_empty(&snapshot.conversation_id, "conversation identifier cannot be empty")?;
        let supervisor = Self {
    ...
    pub fn create_task(&mut self, task: TaskRecord) -> Result<Vec<SupervisorEvent>, &'static str> {
        ...
        for existing in self.tasks.values().filter(|candidate| !candidate.status.terminal()) {
    ...
    pub fn transition(&mut self, task_id: &str, next: TaskStatus) -> Result<SupervisorEvent, &'static str> {
        ...
        task.status = next;
        task.revision = task.revision.checked_add(1).ok_or("task revision overflow")?;
        let task = task.clone();
        self.touch(task_id);
        self.bump()?;
    ...
    pub fn reprioritize(&mut self, task_id: &str, priority: i32) -> Result<SupervisorEvent, &'static str> {
        let task = self.tasks.get_mut(task_id).ok_or("task does not exist")?;
    ...
    pub fn update_constraints(&mut self, task_id: &str, constraints: Vec<String>) -> Result<SupervisorEvent, &'static str> {
    ...
    pub fn request_approval(&mut self, binding: ApprovalBinding) -> Result<Vec<SupervisorEvent>, &'static str> {
        validate_approval(&binding)?;
        let task = self.tasks.get_mut(&binding.task_id).ok_or("approval task does not exist")?;
        if task.status.terminal() {
        ...
        self.bump()?;
        Ok(vec![SupervisorEvent::ApprovalRequested { binding }, SupervisorEvent::TaskUpdated { task: updated }])
    }

    pub fn resolve_approval(&mut self, task_id: &str, approval_id: &str, approved: bool) -> Result<Vec<SupervisorEvent>, &'static str> {
        let task = self.tasks.get_mut(task_id).ok_or("approval task does not exist")?;
        let binding = task.pending_approval.as_ref().ok_or("task has no pending approval")?;
        if binding.task_id != task_id || binding.approval_id != approval_id {
            return Err("approval is not bound to this task and action");
        }
        task.pending_approval = None;
        task.status = if approved { TaskStatus::Active } else { TaskStatus::Blocked };
        task.revision = task.revision.checked_add(1).ok_or("task revision overflow")?;
        let updated = task.clone();
        self.touch(task_id);
        self.bump()?;
        Ok(vec![
            SupervisorEvent::ApprovalResolved { task_id: task_id.to_owned(), approval_id: approval_id.to_owned(), approved },
            SupervisorEvent::TaskUpdated { task: updated },
        ])
    }

    pub fn cancel_task(&mut self, task_id: &str, disposition: CancellationDisposition) -> Result<SupervisorEvent, &'static str> {
        let task = self.tasks.get_mut(task_id).ok_or("task does not exist")?;
        if task.status.terminal() {
    ...
    pub fn cancel_all(&mut self, disposition: CancellationDisposition) -> Result<Vec<SupervisorEvent>, &'static str> {
        let ids: Vec<String> = self.tasks.values().filter(|task| !task.status.terminal()).map(|task| task.id.clone()).collect();
        let mut events = vec![SupervisorEvent::CancelAllRequested { task_ids: ids.clone() }];
        for id in ids {
    ...
    pub fn resolve_reference(&self, reference: &str) -> ReferenceResolution {
        ...
        if matches!(normalized.as_str(), "it" | "that" | "current" | "latest" | "recent") {
            return self.recent_task_ids.last().cloned().map(ReferenceResolution::Resolved).unwrap_or(ReferenceResolution::Missing);
        }
        if self.tasks.contains_key(reference) {
            return ReferenceResolution::Resolved(reference.to_owned());
        }
        let matches: Vec<String> = self.tasks.values()
            .filter(|task| task.name.to_ascii_lowercase() == normalized || task.name.to_ascii_lowercase().contains(&normalized))
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        require_non_empty(&self.conversation_id, "conversation identifier cannot be empty")?;
        ...
        if self.recent_task_ids.iter().any(|id| !self.tasks.contains_key(id)) {
    ...
    fn touch(&mut self, task_id: &str) {
        self.recent_task_ids.retain(|candidate| candidate != task_id);
...
fn valid_transition(current: &TaskStatus, next: &TaskStatus) -> bool {
    ...
    matches!((current, next),
        ...
        (Active, Blocked | ApprovalRequired | Paused | Cancelling | Completed | Failed) |
        (Blocked, Active | Paused | Cancelling | Failed) |
        (ApprovalRequired, Active | Blocked | Paused | Cancelling | Failed) |
...
fn validate_approval(binding: &ApprovalBinding) -> Result<(), &'static str> {
    ...
    if binding.action_fingerprint.len() != 64 || !binding.action_fingerprint.bytes().all(|byte| byte.is_ascii_hexdigit()) {
...
mod tests {
    ...
    fn task(id: &str, name: &str, files: &[&str]) -> TaskRecord {
        TaskRecord {
            id: id.to_owned(), name: name.to_owned(), objective: format!("Complete {name}"), constraints: Vec::new(), owner: None,
            dependencies: BTreeSet::new(), priority: 0, status: TaskStatus::Queued, phase: "planning".to_owned(),
            resources: ResourceClaims { files: files.iter().map(|value| (*value).to_owned()).collect(), ..ResourceClaims::default() },
            pending_approval: None, output_summary: None, cancellation_disposition: None, revision: 0,
        }
    }
    ...
    fn independent_tasks_transition_without_affecting_each_other() {
        ...
        supervisor.create_task(task("a", "Escape bug", &["src/input.rs"])).unwrap();
        supervisor.create_task(task("b", "Session investigation", &["src/session.rs"])).unwrap();
    ...
    fn overlapping_files_emit_conflict_and_are_queryable() {
        let mut supervisor = ConversationalSupervisor::new("conversation").unwrap();
        supervisor.create_task(task("a", "First", &["src/lib.rs"])).unwrap();
        let events = supervisor.create_task(task("b", "Second", &["src/lib.rs"])).unwrap();
        assert!(events.iter().any(|event| matches!(event, SupervisorEvent::ConflictDetected { files, .. } if files == &["src/lib.rs"] )));
        assert_eq!(supervisor.conflicts_for("b").unwrap(), vec![("a".to_owned(), vec!["src/lib.rs".to_owned()])]);
    }
    ...
    fn approvals_cannot_leak_between_tasks() {
        ...
        supervisor.request_approval(ApprovalBinding { approval_id: "approve-a".to_owned(), task_id: "a".to_owned(), action_fingerprint: "a".repeat(64), summary: "write file".to_owned() }).unwrap();
        assert_eq!(supervisor.resolve_approval("b", "approve-a", true), Err("task has no pending approval"));
        assert_eq!(supervisor.resolve_approval("a", "wrong", true), Err("approval is not bound to this task and action"));
    ...
    fn cancel_all_marks_only_non_terminal_tasks_and_records_disposition() {
        ...
        supervisor.cancel_all(CancellationDisposition::ArchiveChanges).unwrap();
        ...
        assert_eq!(supervisor.task("b").unwrap().cancellation_disposition, Some(CancellationDisposition::ArchiveChanges));
    }

    #[test]
    fn references_resolve_by_id_name_recent_and_report_ambiguity() {
        let mut supervisor = ConversationalSupervisor::new("conversation").unwrap();
        supervisor.create_task(task("a", "Session persistence", &[])).unwrap();
        supervisor.create_task(task("b", "Session cleanup", &[])).unwrap();
        assert_eq!(supervisor.resolve_reference("a"), ReferenceResolution::Resolved("a".to_owned()));
        assert_eq!(supervisor.resolve_reference("cleanup"), ReferenceResolution::Resolved("b".to_owned()));
        assert_eq!(supervisor.resolve_reference("it"), ReferenceResolution::Resolved("b".to_owned()));
        assert_eq!(supervisor.resolve_reference("session"), ReferenceResolution::Ambiguous(vec!["a".to_owned(), "b".to_owned()]));
    }

    #[test]
    fn snapshot_restores_frontend_disconnect_state() {
        let mut supervisor = ConversationalSupervisor::new("conversation").unwrap();
        supervisor.create_task(task("a", "Persistent task", &[])).unwrap();
        supervisor.transition("a", TaskStatus::Active).unwrap();
        ...
        assert_eq!(restored.resolve_reference("recent"), ReferenceResolution::Resolved("a".to_owned()));
    }
}
