...
adr = replace_once(
    adr,
    "The headless CLI and interactive TUI consume the canonical stream for durable transcript, plan, question, activity, usage, cancellation, failure, and completion state. Daemon attachments and replay project the same journal range according to each attached frontend kind and expose a next canonical cursor even when every scanned event is non-presentable. Daemon clients consume those projected envelopes directly and acknowledge the batch cursor after hidden events, rather than re-projecting raw journal payloads. The TUI keeps process-local settings, startup recovery, turn-counter, and reset hints only as bounded compatibility inputs while daemon wire integration is completed.",
    "The headless CLI and interactive TUI consume the canonical stream for durable transcript, plan, question, activity, usage, cancellation, failure, and completion state. Daemon attachments and replay project the same journal range according to each attached frontend kind and expose a next canonical cursor even when every scanned event is non-presentable. Daemon protocol v2 carries boxed `FrontendCommandEnvelope`s over the generic local socket and returns typed acknowledgements from one daemon-owned control plane that survives client reconnects. Daemon clients consume those projected envelopes directly and acknowledge the batch cursor after hidden events, rather than re-projecting raw journal payloads. The TUI keeps process-local settings, startup recovery, turn-counter, and reset hints only as bounded compatibility inputs while desktop migration is completed.",
    str(adr_path),
)
