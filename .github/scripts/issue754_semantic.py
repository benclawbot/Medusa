...
if "pub(crate) fn semantic_summary_request" not in text:
    ...
    ModelRequest {
        ...
        messages: vec![Message { role: Role::User, content: vec![MessageBlock::Text { text: payload }] }],
        tools: Vec::new(),
        max_tokens: 1_200,
        temperature_milli: 0,
    }
...
pub(crate) fn validate_semantic_response(
    ...
    let arrays = [
        &history.goal_context, &history.constraints_preferences, &history.completed_work,
        &history.in_progress_work, &history.blockers, &history.key_decisions,
        &history.exact_identifiers, &history.unresolved_questions, &history.next_steps,
    ];
    ...
    if entries > 96 || chars > 32_000 || arrays.iter().flat_map(|values| values.iter()).any(|value| value.chars().count() > 2_000) {
        return None;
    }
...
pub(crate) fn compact_session_with_semantic(
    ...
    append_event(session, Actor::Coordinator, EventPayload::ConversationCompacted {
...
t = engine.read_text()
old_method = '''    /// Replaces prior message history with a bounded durable summary for the next model request.
    pub fn compact_session(
        &self,
        session: &mut AgentSession,
        focus: Option<&str>,
...
new_method = '''    /// Replaces prior history with a V2 manifest and bounded semantic continuity.
    pub fn compact_session(
        &self,
        session: &mut AgentSession,
        focus: Option<&str>,
    ) -> MedusaResult<()> {
        self.compact_session_v2(session, focus)
    }

    fn compact_session_v2(&self, session: &mut AgentSession, focus: Option<&str>) -> MedusaResult<()> {
        ...
        let semantic = self
            ...
            .ok()
            .and_then(|response| crate::compaction_v2::validate_semantic_response(
                &response,
