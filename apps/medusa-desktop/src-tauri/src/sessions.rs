...
fn apply_session_action(
    summary: &mut DesktopSessionSummary,
    action: Option<&DesktopSessionAction>,
) {
...
pub async fn runtime_delete_sessions(
    ...
    if session_ids.len() > MAX_PAGE_SIZE {
        return Err(format!(
            "cannot delete more than {MAX_PAGE_SIZE} sessions at once"
        ));
    }
    ...
    for session_id in session_ids {
        ...
        if registry.session_in_use(&repo, &session_id)? {
            return Err(format!(
                "session {session_id} is currently active and cannot be deleted"
            ));
        }
...
fn read_session_page_sync(
    ...
    for _attempt in 0..2 {
        ...
        if current_fingerprint == Some(index.fingerprint) {
            ...
            apply_session_action(&mut summary, actions.sessions.get(&session_id));
...
mod tests {
    ...
    fn desktop_session_actions_rename_pin_archive_and_delete_without_mutating_other_sessions() {
        ...
        mutate_session_action(&canonical, "a", |action| action.archived = true).expect("archive");
