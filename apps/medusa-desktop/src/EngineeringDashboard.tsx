...
function Dashboard({ repo }: { repo: string }) {
  ...
  return <div className="engineering-dashboard">
    <header className="engineering-header"><div><span className="eyebrow">Medusa engineering system</span><h2>Factory Dashboard</h2><p>Measured outcomes, recurring friction, and guarded self-improvement.</p><small>Imported engineering records are read-only compatibility candidates. Use the shared learning commands for lifecycle changes.</small></div><div className="dashboard-actions"><select value={days} onChange={(event)=>setDays(Number(event.target.value))}><option value={30}>30 days</option><option value={90}>90 days</option><option value={365}>1 year</option></select><button onClick={()=>void reload()} disabled={busy}><RefreshCw size={15}/>Refresh</button></div></header>
    {error && <div className="engineering-error">{error}</div>}
    {data && <><div className="engineering-kpis"><div><span>Task success</span><strong>{data.totalTasks > 0 ? pct(data.successRate) : "No data"}</strong><small>{data.totalTasks > 0 ? `${data.successfulTasks}/${data.totalTasks} successful` : "No completed task data"}</small></div><div><span>Verification pass</span><strong>{pct(data.verificationPassRate)}</strong><small>Recorded verification events</small></div><div><span>Average retries</span><strong>{data.averageRetries.toFixed(2)}</strong><small>Per task</small></div><div><span>Human intervention</span><strong>{pct(data.humanInterventionRate)}</strong><small>Tasks paused for input</small></div><div><span>Average duration</span><strong>{data.averageDurationMinutes.toFixed(1)}m</strong><small>Per task</small></div><div><span>Rollback rate</span><strong>{pct(data.rollbackRate)}</strong><small>Compatibility records</small></div></div><TrendChart data={data.trend}/><div className="engineering-columns"><section className="engineering-card"><div className="card-head"><div><span className="eyebrow">Bottlenecks</span><h3>Recurring friction</h3></div><Activity size={18}/></div><div className="friction-list">{data.friction.length ? data.friction.map((item)=><div key={item.category}><span>{item.category}</span><strong>{item.count}</strong><small>{item.sessions.length} affected session(s)</small></div>) : <p className="empty-state">No recurring friction recorded.</p>}</div></section><section className="engineering-card"><div className="card-head"><div><span className="eyebrow">Guarded evolution</span><h3>Imported candidates</h3></div><ShieldCheck size={18}/></div><div className="proposal-list">{data.improvements.length ? data.improvements.slice().reverse().map((item)=><article key={item.id}><div className="proposal-title"><strong>{item.title}</strong><span className={`status ${item.status}`}>{item.status}</span></div><p>{item.problem}</p><small>{item.proposedChange}</small>{item.benchmarkBefore != null && <div className="benchmark-row"><span>Baseline {pct(item.benchmarkBefore)}</span>{item.benchmarkAfter != null && <span>After {pct(item.benchmarkAfter)}</span>}</div>}<small>Read-only compatibility record. Lifecycle changes must use /learning commands.</small></article>) : <p className="empty-state">No imported candidates.</p>}</div></section></div></>}
  </div>;
}
